...
def build(args):
    ...
    if args.llm:
        try:
            ...
            nt += "\n\n" + P.llm_chat(
                "Write 120 words of plain-text speaker notes explaining, for a "
                "technical advisor, why the planner's route differs from the "
                "human route and what the trade-off is. Use only these facts:\n"
                + ep.to_string() + "\n" + (legs.to_string() if legs is not None else ""))
        except Exception as e:
            nt += f"\n(LLM narrative unavailable: {e})"
    notes(s, nt)

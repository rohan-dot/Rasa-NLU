...
def _sanitize_poc(code: str) -> str:
    ...
    Post-generation sanitizer:
      1. Remove non-standard #include lines the LLM hallucinated.
      2. Fix truncated code (unclosed braces) caused by token-limit cutoffs.
      3. Remove dangling trailing lines that aren't valid C (partial statements).
    """
    # ── Step 1: Strip hallucinated includes ────────────────────────────────
    lines = code.split("\n")
    cleaned = []
    removed = []
    ...
    # ── Step 2: Fix truncated code (token-limit cutoff) ────────────────────
    # If the LLM ran out of tokens mid-function, braces will be unbalanced.
    # Strip any trailing partial line (no semicolon, no brace, no comment)
    # then close the missing braces.
    lines = code.rstrip().split("\n")

    # Remove trailing partial statement (line that doesn't end with ; } { or //)
    while lines:
        last = lines[-1].strip()
        if not last:
            lines.pop()              # blank trailing line
            continue
        if last.endswith((";", "{", "}", "*/", "//")) or last.startswith(("//", "/*", "#")):
            break                    # line looks complete
        print(f"  [sanitize] Removing truncated trailing line: {last[:60]}")
        lines.pop()
    ...
    if open_braces > 0:
        print(f"  [sanitize] Fixing {open_braces} unclosed brace(s) (token-limit truncation)")
        # Add a return 0 before closing if we're likely inside main()
        if open_braces == 1 and "int main" in code:
            code += "\n    return 0;\n}\n"
            open_braces -= 1
        # Close any remaining
        if open_braces > 0:

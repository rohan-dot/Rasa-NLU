...
def compare_reference(stops, ref_overflights=None, blocks_file=None):
    ...
    x = range(len(df))
    def leg_label(row):
        ro = row.ref_overflights_detected or "open water"
        po = row.planner_overflights or "open water"
        return (f"{row.leg}\nhuman flies over: {ro}\n"
                f"planner flies over: {po}")
    labels = [leg_label(r) for _, r in df.iterrows()]
    ax[0].bar([i - .2 for i in x], df.ref_gc_nm, width=.4,
              label="human's straight line", color="#999999")
    ax[0].bar([i + .2 for i in x], df.planner_nm, width=.4,
              label="planner's corridor", color="#4c72b0")
    ax[0].set_xticks(list(x)); ax[0].set_xticklabels(labels, fontsize=8)
    ax[0].set_ylabel("nautical miles"); ax[0].legend()
    ax[0].set_title("Distance of each leg")
    ax[1].bar([i - .2 for i in x], df.ref_clearances_detected, width=.4,
              label="human's straight line", color="#999999")
    ax[1].bar([i + .2 for i in x], df.planner_clearances, width=.4,
              label="planner's corridor", color="#4c72b0")
    ax[1].set_xticks(list(x)); ax[1].set_xticklabels(labels, fontsize=8)
    ax[1].set_ylabel("countries needing overflight clearance")
    ax[1].legend(); ax[1].set_title("Clearances on each leg")
    fig.suptitle("TEST 1 - same stops, different paths between them\n"
                 "Stops fixed to the human route: " + " -> ".join(stops)
                 + ". The planner only chooses how to fly each leg.",
                 fontsize=11)
    fig.tight_layout()
...
def endpoints_compare(stops, aircraft, blocks_file=None):
    ...
    for k, (key, title) in enumerate([("total_nm", "Total distance (NM)"),
                                       ("clearances", "Clearances needed"),
                                       ("landings", "Landings en route")]):
        a = fig.add_subplot(gs[0, k])
        bars = a.bar(["human route", "planner route"], [s1[key], s2[key]],
                     color=["#999999", "#4c72b0"])
        for b_ in bars:
            a.text(b_.get_x() + b_.get_width() / 2, b_.get_height(),
                   f"{b_.get_height():,.0f}", ha="center", va="bottom",
                   fontsize=9)
        a.set_title(title)
    a = fig.add_subplot(gs[1, :]); a.axis("off")
    def chain_text(rows_, label):
        legs = "\n".join(f"   {r['leg']}: {r['nm']:,.0f} NM, flies over "
                          f"{r['overflights'] or 'open water'}, lands {r['lands_in']}"
                          for r in rows_)
        return f"{label}:\n{legs}"
    a.text(0.0, 1.0, chain_text(r1, "HUMAN chose these stops"),
           va="top", ha="left", fontsize=9, family="monospace")
    a.text(0.5, 1.0, chain_text(r2, "PLANNER chose these stops"),
           va="top", ha="left", fontsize=9, family="monospace")
    fig.suptitle(f"TEST 2 - endpoints only: both given {src_ap} -> {dst_ap} "
                 f"and a {spec['type']} (range {rng:,.0f} NM); each chose its "
                 f"own stops\nscore = NM + 300 per clearance + 400 per "
                 f"landing: human {s1['score']:,.0f} vs planner "
                 f"{s2['score']:,.0f}", fontsize=11)
    fig.tight_layout()

...
def _tokens(standard: int) -> int:
    """Scale token limit for thinking models."""
...
[{"file":"f.c","function":"func","bug_type":"overflow",
  ...
  "audit_priority":"critical"}]"""
        ...
        response = self.llm.chat(
            ...
            user=summary, max_tokens=_tokens(2000), temperature=0.2,
        )
        if not response:
            return []
...
[{"function":"f","file":"f.c","bug_type":"overflow",
  "description":"specific explanation","severity":"high"}]""",
                user=user_msg, max_tokens=_tokens(2000), temperature=0.2,
            )
            if not response:
                continue
...
Output ONLY the Python script. No markdown. Start with a comment.""",
            user=user_msg, max_tokens=_tokens(2000), temperature=0.6,
        )
        if not response:
            return 0

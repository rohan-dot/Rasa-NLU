...
BUDGET_FCG = int(os.environ.get("FCG_BUDGET_FCG", "9000"))
...
SYSTEM_PROMPT = (
    ...
    "field, answer ONLY with text quoted verbatim from the dossier (you may "
    "join multiple short verbatim snippets with ' | '). If the dossier does "
    "not contain the answer, respond with the literal string \"NA\". Never "
    "infer, summarize, or use outside knowledge. Respond ONLY with a single "
    "JSON object mapping each field name exactly as given to its answer "
...
def llm_extract(dossier: str, fields):
    ...
    for attempt in range(1, RETRIES + 1):
        try:
            ...
            return parse_json_answer(content, [c for c, _ in fields])
        except Exception as e:
            last_err = e
...
def parse_json_answer(content: str, expected_cols):
    ...
    try:
        ...
        raise RuntimeError(f"Unparseable JSON from model ({e}): {content[:200]}")

...
class PubMedClient:
    """NCBI E-utilities client with proper rate limiting and 429 retry."""

    def __init__(self, api_key: str | None = None):
        self.session = requests.Session()
        self.api_key = api_key
        self._last_request = 0.0
        # NCBI enforces 3 req/sec without key, 10/sec with key
        # We go slower to be safe — 1 req/sec without, 0.2 with
        self._min_interval = 0.2 if api_key else 1.0
        self._backoff = 1.0  # grows on 429s
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if resp.status_code == 429:
                    ...
                    log.warning("  429 rate limited — waiting %.1fs "
                ...
                log.warning("  Request failed (attempt %d): %s",
                            attempt + 1, e)
    ...
    def search(self, query: str, max_results: int = 15) -> list[str]:
        ...
        try:
            ...
            log.warning("PubMed search parse failed: %s", e)
            return []
    ...
    def fetch(self, pmids: list[str]) -> list[Article]:
        ...
        try:
            ...
            log.warning("PubMed fetch parse failed: %s", e)
            return []
...
class BioASQAgent:
    ...
    def _agentic_retrieve(self, question: str, qtype: str,
        ...
        # Also ingest gold document PMIDs — batch fetch in one call
        gold_pmids = []
        for doc_url in raw_question.get("documents", []):
            m = re.search(r'/pubmed/(\d+)', doc_url)
            if m and m.group(1) not in pool.seen_pmids:
                gold_pmids.append(m.group(1))

        if gold_pmids:
            arts = self.pubmed.fetch(gold_pmids)  # single API call
            for a in arts:

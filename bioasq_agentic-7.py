...
class EmbeddingEngine:
    def __init__(self, model_name="pritamdeka/S-PubMedBert-MS-MARCO",
        ...
        log.info("Embedding dim: %d on %s", self.dim, device)
...
def prompt_generate_queries(question, previous=None):
    p = ("Generate 3 short PubMed search queries (3-6 words each) "
         "for this question. Plain keywords only — no AND/OR/NOT, no [Mesh], no field tags.\n\n"
         f"QUESTION: {question}\n\n")
    if previous:
        p += "Previous queries didn't work. Use COMPLETELY DIFFERENT terms:\n"
        p += "\n".join(f"  - {q}" for q in previous[-3:]) + "\n\n"
    p += "Write ONLY the 3 queries, numbered 1-3. Nothing else.\n\n1."
    return p
...
def prompt_answer(question, qtype, passages, few_shot):
    ...
    if qtype == "yesno":
        return _prompt_yesno(question, passages, few_shot)
    elif qtype == "factoid":
        return _prompt_factoid(question, passages, few_shot)
    elif qtype == "list":
        return _prompt_list(question, passages, few_shot)
    else:
        return _prompt_summary(question, passages, few_shot)


def _prompt_yesno(question, passages, few_shot):
    """
    Adversarial yes/no prompt. Forces the model to:
    1. Find evidence FOR yes
    2. Find evidence FOR no
    3. Decide which is stronger
    This prevents the model from latching onto the first positive-sounding passage.
    """
    p = (
        "You are an expert biomedical QA system answering a yes/no question.\n\n"
        "CRITICAL INSTRUCTIONS:\n"
        "1. First, find evidence in the passages that supports YES.\n"
        "2. Then, find evidence that supports NO.\n"
        "3. If the evidence clearly supports one side, choose that side.\n"
        ...
        "5. If the evidence is mixed or insufficient, lean toward NO.\n"
        "6. Do NOT assume something works just because it was studied or is 'promising'.\n"
        "'Promising preclinical results' does NOT mean it should be used clinically.\n\n"
    )
...
def _prompt_factoid(question, passages, few_shot):
    """
    Strict factoid prompt. Forces 1-5 word answers.
    Fixes: wordy answers, hallucinated details, missing "unknown".
    """
    p = (
        "You are an expert biomedical QA system.\n\n"
        "STRICT RULES FOR FACTOID ANSWERS:\n"
        "1. EXACT_ANSWER must be 1-5 words MAXIMUM. A specific name, number, or term.\n"
        "2. Use the EXACT terminology from the evidence passages.\n"
        "3. Do NOT explain, elaborate, or add context in the exact answer.\n"
        "4. If the evidence does not contain a clear answer, write: unknown\n"
        "5. Prefer specific named entities: drug names, gene names, disease names, numbers.\n\n"
        "GOOD exact answers: 'transsphenoidal surgery', 'NF1', '45,X', 'palivizumab', "
        "'30-50 years', 'X-inactivation', 'unknown'\n"
        "BAD exact answers: 'multiple causative factors', 'bleeding during pregnancy or "
        "prior miscarriage', 'transcriptional silencing and heterochromatinization'\n\n"
    )
    for ex in few_shot[:2]:
        p += f"---\nQUESTION: {ex['body']}\nEVIDENCE:\n{_sblock(ex['snippets'], 600)}\n"
        p += f"EXACT_ANSWER: {_fmt_exact(ex['exact_answer'])}\n"
...
def _prompt_list(question, passages, few_shot):
    """
    Exhaustive list prompt. Forces the model to extract ALL items.
    Fixes: returning 3 items when gold has 15+.
    """
    p = (
        "You are an expert biomedical QA system.\n\n"
        "STRICT RULES FOR LIST ANSWERS:\n"
        "1. List EVERY item mentioned in the evidence. Be EXHAUSTIVE.\n"
        "2. It is much better to include too many items than too few.\n"
        "3. Go through EACH evidence passage and extract ALL relevant items.\n"
        "4. Each item should be a specific name or term (1-5 words).\n"
        "5. Include items even if you're not 100% sure they're relevant.\n"
        "6. Aim for at least 5-10 items. Many questions have 10-20+ correct answers.\n"
        "7. Prefix each item with '- ' on its own line.\n\n"
    )
    for ex in few_shot[:1]:  # Only 1 example to save tokens for more evidence
    ...
    p += (
        f"---\nQUESTION: {question}\n"
        f"EVIDENCE:\n{_sblock(passages, 5000)}\n"  # More evidence for lists
        "Now list EVERY relevant item from the evidence. Be EXHAUSTIVE:\n\n"
        "EXACT_ANSWER:\n"
    )
    return p


def _prompt_summary(question, passages, few_shot):
    p = (
        "You are an expert biomedical QA system. Write a comprehensive "
        "3-6 sentence paragraph answering the question using the evidence.\n\n"
    )
    for ex in few_shot[:2]:
...
def parse_yesno(r):
    # The adversarial prompt outputs: EVIDENCE FOR YES: ... EVIDENCE FOR NO: ... EXACT_ANSWER: ...
    # Find the EXACT_ANSWER part
    ea_match = re.search(r'EXACT_ANSWER\s*:\s*(.*?)(?:\n|IDEAL|$)', r, re.IGNORECASE)
    if ea_match:
        exact_raw = ea_match.group(1).strip().lower().strip('"\'').rstrip(".")
    else:
        ...
        for line in reversed(lines):
            ...
            if "yes" in line[:10] or "no" in line[:10]:
                exact_raw = line
                break
        if not exact_raw:
            exact_raw = lines[-1].strip().lower() if lines else ""
    ...
    # Determine yes/no
    if "yes" in exact_raw[:10] and "no" not in exact_raw[:5]:
        exact = "yes"
    elif "no" in exact_raw[:10]:
        exact = "no"
    else:
        # Scan the reasoning for the conclusion
        lower_r = r.lower()
        # Count evidence signals
        no_signals = len(re.findall(r'insufficient|toxicity|not effective|no evidence|failed|ineffective|not recommended|lack of', lower_r))
        yes_signals = len(re.findall(r'effective|demonstrated|shown to|evidence supports|approved|recommended', lower_r))
        exact = "no" if no_signals >= yes_signals else "yes"
    ...
    def _retrieve(self, question, qtype, raw_q, index, tools):
        ...
        for it in range(self.max_iters):
            ...
            for line in qr.strip().split("\n"):
                ...
                # Filter out preamble / non-query text
                if not line or len(line) < 5 or len(line) > 80:
                    continue
                # Skip lines that look like explanations, not queries
...
def main():
    ...
    parser.add_argument("--embed-device", default=None,
                        help="Device for embedding model: cuda, cuda:1, cpu (default: auto)")
    ...
    embedder = EmbeddingEngine(model_name=args.embedding_model, device=args.embed_device)
    ...
    # ---- Write Phase B submission (exact + ideal answers) ----
    phase_b = {"questions": []}
    for r in results:
    ...
    log.info("Phase B submission: %s", out_b)

    # ---- Write Phase A submission (documents + snippets) ----
    phase_a = {"questions": []}
    for r in results:
        entry = {"id": r["id"]}
        entry["documents"] = r.get("documents", [])
        # Fix snippet format: BioASQ expects "sections.0" not "abstract"
        snippets = []
        for s in r.get("snippets", []):
            sec = s.get("beginSection", "abstract")
            if sec == "abstract": sec = "sections.0"
            elif sec == "title": sec = "sections.0"
            snippets.append({
                "document": s.get("document", ""),
                "text": s.get("text", ""),
                "offsetInBeginSection": s.get("offsetInBeginSection", 0),
                "offsetInEndSection": s.get("offsetInEndSection", 0),
                "beginSection": sec,
                "endSection": sec,
            })
    ...
    out_a = out_b.replace("phaseB", "phaseA").replace(".json", "_phaseA.json") \
            if "phaseB" in out_b else out_b.replace(".json", "_phaseA.json")
    with open(out_a, "w") as f:
        json.dump(phase_a, f, indent=2, ensure_ascii=False)
    log.info("Phase A submission: %s", out_a)

    print(f"\nDone! {len(results)} questions")
    print(f"Types: {dict(Counter(q.get('type','?') for q in questions))}")
    print(f"Phase B (answers):  {out_b}")

    ...
    def generate(self, prompt: str) -> str:
        ...
        for attempt in (1, 2):
            try:
                ...
                text = self.tok.decode(out[0][input_ids.shape[1]:],
...
def main():
    ...
    # Resume support: countries already in the output CSV are skipped, and each
    # finished country is appended immediately (crash-safe for long runs).
    done = set()
    ...
    fx.log(f"Done: {len(rows)} new countries appended -> {args.out} "
           f"({len(done)} were already present)")

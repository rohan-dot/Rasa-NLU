...
class BM25Searcher:
    """BM25 keyword search — tries bm25s directory, pickle file, then gives up gracefully."""
    def __init__(self,index_path):
        ...
        # Try bm25s (expects a directory)
        try:
            import bm25s
            self.bm25=bm25s.BM25.load(index_path,load_corpus=False)
            self.backend="bm25s";self.active=True
            log.info("BM25 loaded (bm25s)")
            return
        except ImportError:
            log.warning("bm25s not installed — pip install bm25s PyStemmer")
        except Exception as e:
            log.warning("bm25s load failed: %s",e)
        # Try pickle
        try:
            import pickle
            path = index_path if not index_path.endswith('/') else index_path.rstrip('/')
            with open(path,"rb") as f:self.bm25=pickle.load(f)
            self.backend="pickle";self.active=True
            log.info("BM25 loaded (pickle)")
            return
        except Exception as e2:
            log.warning("Pickle load failed: %s",e2)
        log.warning("BM25 unavailable — running FAISS only")
...
class CorpusDB:
    def __init__(self,db_path):
        ...
        # Check for abstract-like columns: abstract, body, text, content
        self.abstract_col=next((c for c,cl in zip(cols,cols_l)
                                if any(x in cl for x in ("abstract","body","text","content"))),None)
    ...
    def keyword_search(self, query, k=20):
        """Fallback keyword search using SQLite LIKE — slower but works without BM25 index."""
        ...
        # Build WHERE clause: all terms must appear in title or abstract
        col = self.abstract_col or self.title_col or "body"
        where = " AND ".join(f"({col} LIKE ? OR {self.title_col or col} LIKE ?)" for _ in terms)
        params = []
        for t in terms:
        ...
        try:
            rows = conn.execute(
                f"SELECT rowid, * FROM {self.table} WHERE {where} LIMIT ?",
                params + [k]
            ).fetchall()
            results = []
            for row in rows:
                d=dict(row)
                pmid=str(d.get(self.pmid_col,""))
                title=str(d.get(self.title_col,"") or "") if self.title_col else ""
                abstract=str(d.get(self.abstract_col,"") or "") if self.abstract_col else ""
                if not abstract:
                    for v in d.values():
...
class HybridSearcher:
    ...
    def search(self,query,k=30):
        """Hybrid search: FAISS + BM25 (or corpus keyword fallback) with RRF."""
        ...
        if bm:
            # Full hybrid
            for r,(i,_) in enumerate(sem):rrf[i]=rrf.get(i,0)+0.6/(K+r+1)
            for r,(i,_) in enumerate(bm):rrf[i]=rrf.get(i,0)+0.4/(K+r+1)
        else:
            # FAISS only + corpus keyword fallback
            for r,(i,_) in enumerate(sem):rrf[i]=rrf.get(i,0)+0.7/(K+r+1)
            # Keyword fallback from corpus DB
            kw_results = self.corpus.keyword_search(query, k=15)
            for r,art in enumerate(kw_results):
                idx = art["_idx"]
                rrf[idx] = rrf.get(idx,0) + 0.3/(K+r+1)

        ranked=sorted(rrf.items(),key=lambda x:-x[1])[:k]
        idx_map=dict(ranked)
        arts=self.corpus.fetch_by_indices([i for i,_ in ranked])
    ...
    def multi_search(self, queries, k=30):
        """Search with multiple queries and merge results."""
        all_passages = []
        seen_sigs = set()
        for q in queries:
            results = self.search(q, k=k)
            for p in results:
                sig = p.text[:80].lower()
...
def _fac(q,ts,fs):
    p=("You are an expert biomedical QA system.\n\n"
       ...
       "1. EXACT_ANSWER must be 1-5 words MAXIMUM.\n"
       "2. COPY the exact term/name/number from the evidence — do NOT paraphrase.\n"
       "3. If the evidence says '3-8%', write '3-8%' — NOT 'approximately 5%'.\n"
       "4. If the evidence says 'POLR2M', write 'POLR2M' — NOT 'RNA polymerase subunit'.\n"
       "5. If the evidence says a specific gene/protein/drug name, use that EXACT name.\n"
       "6. If no clear answer exists in the evidence, write: unknown\n"
       "7. Then write IDEAL_ANSWER: 2-4 sentences (max 200 words).\n\n"
       "GOOD: '3-8%', 'POLR2M', 'transsphenoidal surgery', '24.5 years', 'membrane pore forming'\n"
       "BAD: 'approximately 5%', 'a subunit of RNA polymerase', 'surgical treatment', "
       "'about 25 years', 'helps with cell membrane'\n\n")
    for ex in fs[:2]:
        p+=f"---\nQ: {ex['body']}\nEVIDENCE:\n{_sb(ex['snippets'],600)}\n"
        p+=f"EXACT_ANSWER: {_fmt(ex['exact_answer'])}\nIDEAL_ANSWER: {ex['ideal_answer']}\n\n"
    p+=f"---\nQ: {q}\nEVIDENCE:\n{_sb(ts)}\n"
    p+="Copy the exact term from the evidence:\nEXACT_ANSWER:"
    return p
def _lst(q,ts,fs):
    p=("You are an expert biomedical QA system.\n\n"
       "STRICT RULES FOR LIST ANSWERS:\n"
       "1. Go through EACH evidence passage ONE BY ONE.\n"
       "2. From each passage, extract EVERY relevant item.\n"
       "3. It is CRITICAL to list ALL items — missing items loses points.\n"
       "4. Aim for 10-20+ items. Most questions have 10+ correct answers.\n"
       "5. Each item: 1-5 words, specific name/term. Prefix '- '.\n"
       "6. Include items even if similar — 'diazepam' and 'lorazepam' are separate items.\n"
       "7. For diseases: list each disease separately.\n"
       "8. For drugs: list each drug separately.\n"
       "9. For genes/proteins: list each one separately.\n"
       "10. After the full list, write IDEAL_ANSWER: 2-4 sentences (max 200 words).\n\n"
       "EXAMPLE: If asked 'What drugs treat X?' and evidence mentions 5 drugs across "
       "3 passages, list ALL 5 — not just 2-3.\n\n")
    for ex in fs[:1]:
        p+=f"---\nQ: {ex['body']}\nEVIDENCE:\n{_sb(ex['snippets'],500)}\nEXACT_ANSWER:\n"
        ea=ex["exact_answer"]
        if isinstance(ea,list):
    ...
    p+=f"---\nQ: {q}\nEVIDENCE:\n{_sb(ts,6000)}\n\n"
    p+="Go through each passage and extract ALL items:\n\nEXACT_ANSWER:\n"
    return p
def _sum(q,ts,fs):
...
class Agent:
    ...
    def solve(self,question):
        ...
        for sq_i,sub_q in enumerate(sub_qs):
            ...
            # Direct search with question + keyword variant
            kw_q = simple_kw(sub_q)
            search_queries = [sub_q]
            if kw_q and kw_q != sub_q: search_queries.append(kw_q)

            trace.append(ReActStep("ACT",f"SEARCH_HYBRID('{sub_q[:50]}...')",tool="SEARCH_HYBRID"))
            r=self.searcher.multi_search(search_queries, k=25)
            all_p.extend(r)
            ...
            # LLM queries — more iterations for list questions
            max_iters = self.iters + 1 if qtype == "list" else self.iters
            for it in range(max_iters):
                ...
                # More evidence needed for list questions
                min_passages = 30 if qtype == "list" else 20
                if len(all_p)>=min_passages:
        ...
        # For list questions: second pass to catch missed items
        if qtype == "list" and ec:
            current_items = ec[0] if ec else []
            items_str = ", ".join(current_items[:20])
            second_pass = self.llm.ask(
                f"You previously listed these items for the question:\n"
                f"Q: {body}\n\nCURRENT LIST: {items_str}\n\n"
                f"EVIDENCE:\n{_sb(ptexts, 5000)}\n\n"
                f"Are there ANY items in the evidence that you MISSED? "
                f"Go through each passage carefully. List ONLY the NEW items "
                f"not already in the current list, prefixed with '- '.\n"
                f"If nothing is missing, write: NONE\n\nMISSED ITEMS:\n",
                max_tokens=512, temp=0.2)
            if "NONE" not in second_pass.upper():
                extra_items, _ = p_lst(second_pass)
                if extra_items and extra_items != ["unknown"]:
                    ec.append(extra_items)
                    log.info("  Second pass found %d more items", len(extra_items))
                    trace.append(ReActStep("ACT",
                        f"LIST_REVIEW: +{len(extra_items)} missed items", tool="LIST_REVIEW"))

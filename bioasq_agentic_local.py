...
The agent has 6 tools:
  1. SEARCH_DB(query)       — FTS5 keyword search on local SQLite (33M articles)
  2. SEARCH_SEMANTIC(query) — PubMedBERT embeddings + FAISS cosine similarity
  3. SEARCH_HYBRID(query)   — RRF fusion of both
  4. DECOMPOSE(question)    — Break complex questions into sub-questions
  5. EXPAND(pmid)           — Find related articles by content similarity
  6. SYNTHESIZE(answers)    — Combine sub-answers into final answer
...
class Agent:
    ...
    def __init__(self, llm, db, emb, fs, passes=3, iters=3):
        self.llm = llm
        self.db = db
        self.emb = emb
        self.fs = fs
    ...
    def solve(self, question: dict) -> dict:
        ...
        # ── LLM rerank final passages ──
        top = idx.search_hybrid(body, k=20)
        if len(top) > 5:
            block = "\n".join(
                f"[{i+1}] {p.text[:200]}" for i, p in enumerate(top[:20]))
            sr = self.llm.ask(
                f"Rate relevance (0-2) to: {body}\n\n{block}\n\n[N] SCORE:",
            ...
            for line in sr.strip().split("\n"):
                ...
                if m:
                    ...
                    if 0 <= i < len(top):
                        top[i].similarity_score = (
                            top[i].similarity_score * 0.4 + (sc / 2) * 0.6)
            top = sorted(top, key=lambda p: p.similarity_score, reverse=True)
            trace.append(ReActStep("ACT", "RERANK — LLM re-scored top passages",
                                   tool="RERANK"))
...
def main():
    ...
    agent = Agent(llm, db, emb, fs, a.passes, a.retrieval_iterations)

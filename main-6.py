...
from crs.code_intelligence import build_context, classify_vulnerability
...
def run_task(task, router, evaluator, max_pocs=30):
    ...
    desc_display = task.vulnerability_description[:100] if task.vulnerability_description else "(none — Level 0)"
    print(f"  DESC : {desc_display}...")
    print(f"{'='*60}")

    t0 = time.time()

    # Step 0: Level 0 discovery — if no description, find vulns via LLM
    is_level0 = not task.vulnerability_description or len(task.vulnerability_description.strip()) < 10
    vuln_candidates = []
    if is_level0:
        print(f"\n  Step 0: No description — running vulnerability discovery...")
        from crs.vuln_discovery import discover_vulnerabilities, build_context_from_candidate
        vuln_candidates = discover_vulnerabilities(task, router)
        if not vuln_candidates:
            print(f"  ✗ No vulnerability candidates found")
            evaluator.record(task, "unknown", [], time.time()-t0, False, False)
            return
        print(f"  ✓ Found {len(vuln_candidates)} candidate(s)")
        for i, vc in enumerate(vuln_candidates):
            print(f"    [{i}] {vc.function_name}: {vc.description[:80]}...")
        # Use the top candidate's description for the rest of the pipeline
        best = vuln_candidates[0]
        task.vulnerability_description = best.description
        print(f"\n  Using top candidate: {best.function_name} ({best.vuln_type})")
    ...
    elif is_level0 and len(vuln_candidates) > 1:
        # Level 0: try remaining candidates
        print(f"\n  ✗ No trigger with top candidate — trying others...")
        from crs.vuln_discovery import build_context_from_candidate
        for ci, vc in enumerate(vuln_candidates[1:], 2):
            print(f"\n  --- Trying candidate [{ci}]: {vc.function_name} ---")
            task.vulnerability_description = vc.description
            alt_ctx = build_context(task)
            alt_ctx.description = vc.description
            alt_ctx.vuln_type = classify_vulnerability(vc.description)
            poc_candidates = orchestrator.run(alt_ctx, harness)[:max_pocs]
            if poc_candidates:
                alt_results = execute_pipeline(fuzz_build.binary_path, poc_candidates)
                run_results.extend(alt_results)
                alt_triggered = sum(1 for r in alt_results if r.triggered)
                if alt_triggered:
                    print(f"\n  ✓ SUCCESS with candidate [{ci}]!")
                    triggered = alt_triggered
                    break
    else:
        print(f"\n  ✗ No trigger")

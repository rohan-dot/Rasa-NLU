...
def p_lst(r):
    ...
    for l in lr.split("\n"):
        ...
        # Filter out artifacts and junk
        if not l or len(l) <= 1 or len(l) > 100: continue
        if any(skip in l.upper() for skip in [
            "CORRECTED_ANSWER", "EXACT_ANSWER", "IDEAL_ANSWER",
            "EVIDENCE", "QUESTION", "NOTE:", "LIST"
        ]): continue
        items.append(l)
...
def main():
    ...
    # ── Format exact_answer per BioASQ spec ──
    def format_exact(ea, qtype):
        """
        BioASQ format:
          factoid: [["answer1"], ["answer2"], ...] — up to 5 inner lists, 1 item each
          list:    [["item1"], ["item2"], ...]      — up to 100 inner lists, 1 item each
          yesno:   "yes" or "no"                    — plain string
        """
        if qtype == "yesno":
            return ea  # already a string
        if qtype == "factoid":
            if isinstance(ea, list):
                # Wrap each item in its own list
                return [[x] if isinstance(x, str) else x for x in ea[:5]]
            return [[str(ea)]]
        if qtype == "list":
            if isinstance(ea, list):
                return [[x] if isinstance(x, str) else x for x in ea[:100]]
            return [[str(ea)]]
        return ea
    ...
    for r in results:
        ...
        if "exact_answer" in r and r["exact_answer"] is not None:
            e["exact_answer"] = format_exact(r["exact_answer"], r.get("_type", ""))

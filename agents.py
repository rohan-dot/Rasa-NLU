...
def _tokens(standard: int) -> int:
    """Scale token limit for thinking models (need 2.5x for reasoning chain)."""
    return int(standard * 2.5) if _use_thinking else standard
...
class ScannerAgent:
    ...
    def _scan_one(self, func_name, call_graph, src_dir):
        ...
        response = self.llm.chat(
            ...
            max_tokens=_tokens(1500), temperature=0.1,
        )
        if not response:
            return []
        ...
        for attempt in range(4):
            ...
            if attempt == 0:
                ...
                response = self.llm.chat(
                    ...
                    max_tokens=_tokens(2000), temperature=0.2,
                )
            elif compile_error:
                ...
                response = self.llm.chat(
                    ...
                    max_tokens=_tokens(2000), temperature=0.1 + attempt * 0.1,
                )
                ...
                response = self.llm.chat(
                    ...
                    max_tokens=_tokens(2000), temperature=0.3 + attempt * 0.1,
                )
            else:
                break
...
class VerifierAgent:
    ...
    def verify(self, crash_output, finding, call_graph, src_dir):
        ...
        response = self.llm.chat(system=VERIFIER_PROMPT, user=user_msg,
                                 max_tokens=_tokens(500), temperature=0.1)
        if not response:
            return {"real_vulnerability": True, "severity": "unknown", "verified": False}
    ...
    def check_reachability(self, finding, call_graph, main_binary, src_dir):
        ...
        response = self.llm.chat(
            ...
            max_tokens=_tokens(2000), temperature=0.3,
        )

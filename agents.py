...
finding from a scanner. Your ONLY job is to craft a raw byte input that
triggers this exact bug.

You will receive:
1. The vulnerability details (function, bug type, data flow)
2. The source code of the vulnerable function
3. Feedback from previous attempts (if this is a retry)

Think step by step:
1. What input format does the program accept? (XML for libxml2)
2. How does the input reach the vulnerable function? (follow the data_flow)
3. What exact bytes trigger the bug? (overflow the size, hit the off-by-one, etc.)
4. Encode those bytes as hex.

For XML parser targets, remember:
- XML must be well-enough formed to reach the parser internals
- Namespace prefixes use the colon separator: <prefix:localname>
- Entity declarations: <!DOCTYPE x [<!ENTITY e "value">]>
- Very long strings can overflow fixed buffers
- Null bytes (00) can confuse string length calculations

Respond with ONLY a JSON array of 3-5 PoC inputs:
[{"hex": "3c3f786d6c...", "rationale": "step-by-step explanation of how these bytes reach the bug"}]

Your response must start with [ and end with ] — nothing else."""

EXPLOITER_RETRY_SYSTEM = """\
Your previous PoC inputs did NOT crash the target. Here is what happened:
{feedback}

Analyze why it didn't work and try a COMPLETELY DIFFERENT approach.
Think about what the previous inputs were missing:
- Were they malformed enough that the parser rejected them before reaching the vulnerable code?
- Were the sizes/lengths not large enough to trigger the overflow?
- Did the input format not match what the vulnerable function expects?

Generate 3-5 NEW PoC inputs with a different strategy.
Respond with ONLY a JSON array:
[{{"hex": "...", "rationale": "what's different this time and why it should work"}}]

Your response must start with [ and end with ] — nothing else."""


class ExploiterAgent:
    """Takes a scan finding and tries to craft a crashing input."""
    ...
    def exploit(
        ...
        """Try to exploit a specific finding. Iterates with feedback."""
        if not self.llm.is_available():
        ...
        # Get source context for the vulnerable function
        ...
        for attempt in range(max_attempts):
            ...
            for poc_info in pocs:
                ...
                crashed, output = _run_poc(self.binary_path, data)
                ...
                if crashed:
                    ...
                    logger.info(
                        ...
                        finding.bug_type, finding.function, len(data),
                    )
                    return ExploitResult(
                    ...
                    attempt_feedback.append(
                        f"Input ({len(data)} bytes): no crash. "
                        f"Output: {output[:200]}"
                    )

            feedback = "\n".join(attempt_feedback) if attempt_feedback else "No valid hex inputs generated."

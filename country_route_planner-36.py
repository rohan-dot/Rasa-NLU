...
def build_airport_index(fcg: pd.DataFrame, ourairports: pd.DataFrame):
    ...
    for _, row in fcg.iterrows():
        ...
        for tok in AIRPORT_CODE_RE.findall(text.upper()):
            ...
            if tok.isdigit():
                continue
            # runway designators (02R, 13L, 36C), wind/altitude units (15KT,
            # 49FT), and anything starting with a digit are not airports
            if re.match(r"^\d", tok) or tok.endswith(("KT", "FT", "NM")):
                continue

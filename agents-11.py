...
from triage import skeptical_triage

# Use lld if available (fixes the sancov ordered/unordered linker error
# from bfd when #include-ing source into a fuzzer harness). Falls back
# to the default linker if lld isn't installed (e.g. some dev machines).
_HAS_LLD = shutil.which("ld.lld") is not None or shutil.which("lld") is not None
_LLD_FLAG = ["-fuse-ld=lld"] if _HAS_LLD else []
...
def compile_harness(code: str, name: str, src_dir: str, output_dir: str) -> tuple[str | None, str]:
    ...
    def _build_cmd(extra_defines: list[str] = None, extra_libs: list[str] = None) -> list[str]:
        # -fuse-ld=lld: the GNU bfd linker rejects fuzzer harnesses that
        # #include source ("__sancov_cntrs has both ordered and unordered");
        # lld handles the mixed coverage-counter layout fine.
        # -Wno-* : arbitrary codebases redefine macros / have implicit decls;
        # these must stay warnings, never errors, or harness compiles break.
        cmd = ["clang", "-g", "-O1", "-fsanitize=address,fuzzer", *_LLD_FLAG,
...
class ScannerAgent:
    ...
    def scan_top_targets(self, call_graph, src_dir, max_targets=8, tool_context=""):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
            futures = {
                pool.submit(self._scan_one, name, call_graph, src_dir, tool_context): name
    ...
    def _scan_one(self, func_name, call_graph, src_dir, tool_context=""):
        ...
        # Inject static-analysis / taint / diff evidence from real tools
        if tool_context:
...
class AgentOrchestrator:
    ...
    def __init__(self, llm, call_graph, src_dir, output_dir,
        ...
        self.tool_context = tool_context  # static analysis + taint + diff evidence
        self.all_findings = []
        self.all_exploits = []
    ...
    def run_pipeline(self, max_scan_targets=8, max_exploit_targets=3):
        ...
        # Phase 2: Triage + exploit top findings
        ...
        for finding in top:
            ...
            # ── Skeptical triage (AISLE-style): grep-verify the finding ──
            code_ctx = self.call_graph.get_function_context(finding.function, self.src_dir) or ""
            triage = skeptical_triage(self.llm, finding, code_ctx, self.src_dir, rounds=3)
            if triage.verdict == "VALID":
                logger.info("╔══════════════════════════════════════════╗")
                logger.info("║  *** VALID VULNERABILITY ***             ║")
                logger.info("║  %s in %s", finding.bug_type, finding.function)
                logger.info("║  Confidence: %.0f%% %s",
                            triage.confidence * 100, triage.rounds_str)
                logger.info("║  %s", triage.reasoning[:44])
                logger.info("╚══════════════════════════════════════════╝")
            else:
                logger.info("[triage] %s in %s → %s %s (%.0f%%)",
                            finding.bug_type, finding.function, triage.verdict,
                            triage.rounds_str, triage.confidence * 100)
            ...
            if result.crashed:
                ...
                report = {
                    ...
                    "triage_rounds": triage.rounds_str,
            ...
            elif triage.verdict == "VALID" and triage.confidence >= 0.6:
                # No crash, but triage strongly believes it's real — record as candidate
                report = {
                    "function": finding.function, "file": finding.file,
                    "bug_type": finding.bug_type, "confidence": finding.confidence,
                    "description": finding.description,
                    "classification": "TRIAGE-VALID (no PoV)",
                    "triage_verdict": triage.verdict,
                    "triage_confidence": triage.confidence,
                    "triage_rounds": triage.rounds_str,
                    "triage_reasoning": triage.reasoning,
                    "grep_evidence": triage.grep_evidence,
                    "strategy": "multi_agent_v5",
                    "timestamp": time.time(),
                }
                rh = hashlib.sha256(
                ...
                (self.bugs_dir / f"candidate-{rh}.json").write_text(
                    json.dumps(report, indent=2, default=str)
                )

...
def main():
    ...
    # 3. Recursive search (OSS-CRS may nest the binary in subdirs)
    if not binary_exists and Path(build_dir).is_dir():
        for g in Path(build_dir).rglob(f"*{harness_name}*"):
            if g.is_file() and os.access(g, os.X_OK) and not g.suffix:
                binary_exists = True
                found_binary = str(g)
                log.info("[binary] Found harness in subdir: %s", g)
                break

    # 4. Last resort: any executable in build_dir that looks like a fuzzer
    if not binary_exists and Path(build_dir).is_dir():
        for g in Path(build_dir).rglob("*"):
            if g.is_file() and os.access(g, os.X_OK) and not g.suffix:
                # Skip obvious non-fuzzers
                if any(s in g.name.lower() for s in ["fuzzer", "harness", "_test", harness_name.lower()]):
                    binary_exists = True
                    found_binary = str(g)
                    log.info("[binary] Found candidate fuzzer: %s", g)
                    break

    if binary_exists:
        log.info("[binary] Using existing harness: %s", found_binary)
        build_dir = str(Path(found_binary).parent)
        harness_name = Path(found_binary).name
    else:
        log.warning("[binary] No harness binary found in %s for '%s'. "
                    "Will auto-generate.", build_dir, harness_name)

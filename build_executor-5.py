    ...
    def _attempt(cflags: str, label: str, extra_cmake_flags: list[str] | None = None):
        ...
        try:
            if build_type == "cmake":
                ...
                cmake_source = repo
                # First try source_dir from build_info (set by detect_build_system)
                if "source_dir" in build_info:
                    candidate = Path(build_info["source_dir"])
                    if (candidate / "CMakeLists.txt").exists():
                        cmake_source = candidate

                # If still not found, search recursively
                if not (cmake_source / "CMakeLists.txt").exists():
                    found = sorted(repo.rglob("CMakeLists.txt"))
                    if found:
                        cmake_source = found[0].parent
                        print(f"[build] CMakeLists.txt not at repo root — found at: {cmake_source}")
                    else:
                        log += "\nNo CMakeLists.txt found anywhere in repo"
                        return None, log + f"\nConfigure failed ({label}): no CMakeLists.txt"
                ...
                r = _run_cmd(cmake_cmd, cwd=cmake_source, env=env)
                log += r.stdout.decode(errors="replace") + r.stderr.decode(errors="replace")
                if r.returncode != 0:
                    print(f"[build] Configure FAILED ({label}) — last 1500 chars of log:")
                    print(log[-1500:])
                ...
                if r2.returncode != 0:
                    print(f"[build] cmake --build FAILED ({label}) — last 1500 chars of log:")
                    print(log[-1500:])
                    return None, log + f"\nBuild failed ({label})"
                ...
                log += r.stdout.decode(errors="replace") + r.stderr.decode(errors="replace")
                if r.returncode != 0:
                    print(f"[build] Meson setup FAILED ({label}) — last 1500 chars of log:")
                    print(log[-1500:])
                ...
                log += r2.stdout.decode(errors="replace") + r2.stderr.decode(errors="replace")
                if r2.returncode != 0:
                    print(f"[build] Meson compile FAILED ({label}) — last 1500 chars of log:")
                    print(log[-1500:])
                ...
                log += r2.stdout.decode(errors="replace") + r2.stderr.decode(errors="replace")
                if r2.returncode != 0:
                    print(f"[build] make FAILED ({label}) — last 1500 chars of log:")
                    print(log[-1500:])
                    return None, log + f"\nBuild failed ({label})"
...
def compile_poc(
    ...
    log = ""
    compiled = False
    print(f"[compile] Attempt 1 (with sanitizers): {' '.join(cmd)}")
    try:
        r = _run_cmd(cmd, timeout=BUILD_TIMEOUT, cwd=work)
        log = r.stdout.decode(errors="replace") + r.stderr.decode(errors="replace")
        compiled = r.returncode == 0
        if not compiled:
            print(f"[compile] Attempt 1 FAILED (rc={r.returncode}) — gcc output:")
            print(log[-3000:])
    except subprocess.TimeoutExpired:
        log = "compilation timed out"
        compiled = False
        print(f"[compile] Attempt 1 FAILED — timed out")
    except Exception as exc:
        log = f"compilation error: {exc}"
        compiled = False
        print(f"[compile] Attempt 1 FAILED — exception: {exc}")
    ...
    if not compiled and "-fsanitize" in " ".join(cmd):
        ...
        print(f"[compile] Attempt 2 (no sanitizers): {' '.join(fallback_cmd)}")
        try:
            ...
            compiled = r.returncode == 0
            if not compiled:
                print(f"[compile] Attempt 2 FAILED (rc={r.returncode}) — gcc output:")
                print(fb_log[-3000:])
        except Exception as exc:
            log += f"\nfallback error: {exc}"
            print(f"[compile] Attempt 2 FAILED — exception: {exc}")
    ...
    if not compiled:
        ...
        if vuln_sources:
            ...
            print(f"[compile] Attempt 3 (with {len(vuln_sources)} source files)")
            try:
                ...
                compiled = r.returncode == 0
                if compiled:
                    print("[compile] Attempt 3 OK — compiled with vulnerable sources")
                else:
                    print(f"[compile] Attempt 3 FAILED (rc={r.returncode}) — gcc output:")
                    print(src_log[-3000:])
            except Exception as exc:
                log += f"\nsrc fallback error: {exc}"
                print(f"[compile] Attempt 3 FAILED — exception: {exc}")

    status = "OK" if compiled else "FAILED"
    print(f"[compile] PoC '{safe_name}': {status}")
    if not compiled:
        print(f"[compile] --- error log for '{safe_name}' (last 2000 chars) ---")
        print(log[-2000:])
        print(f"[compile] --- end error log ---")

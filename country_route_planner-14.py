...
AIRPORT_COL  = "Aircraft_Entry_Exit_Airfield_Restrictions"
...
# Overflight columns FIRST — they matter most for corridor countries.
OVERFLIGHT_COLS = [
    "Overflight_Clearance_Requirements",
    "Overflight_Fees",
    "Diplomatic_Clearance_Lead_Time",
    "HazMat_Clearance_Requirements",
]
FEASIBILITY_COLS = OVERFLIGHT_COLS + [
    "Aircraft_Entry_Exit_Airfield_Restrictions",
    "Customs_Procedures",
...
def country_dossier(code, fcg, role=None):
    if code not in fcg.index:
        return "(no FCG data for this country)"
    row = fcg.loc[code]
    cap = DOSSIER_CHAR_CAP or 10 ** 9
    cols = FEASIBILITY_COLS
    if role == "overflight":
        # corridor countries: overflight rules first, ground-ops fields dropped
        cols = OVERFLIGHT_COLS + ["Other_Pertinent_Notes_Msn_or_Info_Remarks",
                                  "Daylight_Only_VFR_Only_Restrictions", "NOTAMs"]
    parts = [f"[{c}]\n{str(row[c]).strip()[:cap]}"
             for c in cols
...
def country_key_facts(code, fcg, role, per_field_cap=600):
    """Compact per-country facts for the operational briefing: the actual FCG
    text (lead times, fees, hours, customs) the summary should cite."""
    if code not in fcg.index:
        return ""
    row = fcg.loc[code]
    cols = (OVERFLIGHT_COLS if role == "overflight" else
            ["Diplomatic_Clearance_Lead_Time", "Overflight_Clearance_Requirements",
             "Aircraft_Entry_Exit_Airfield_Restrictions", "Customs_Procedures",
             "Operating_Hours", "Holiday_Closures",
             "AIR_Card_Acceptance", "Cash_Payment_Requirement",
             "Landing_and_Handling_Fees", "HazMat_Clearance_Requirements"])
    parts = [f"  - {c}: {str(row[c]).strip()[:per_field_cap]}"
             for c in cols
             if c in row.index and pd.notna(row[c]) and str(row[c]).strip()
             and str(row[c]).strip().upper() != "NA"]
    return "\n".join(parts)
...
def judge_country(code, name, role, travel_date, fcg, airport=None,
    ...
    if airport and not str(airport).startswith("CENTROID:"):
        ...
        via = ""
    dossier = country_dossier(code, fcg, role)
    if VERBOSE:
...
def narrate(chosen, total_nm, alternates, stop_countries, corridors,
            verdicts, nodes, ourairports, suggestions, fuel_notes, fcg,
            travel_date, use_llm):
    def ap_name(ap):
        if ap.startswith("CENTROID:"):
            c = ap.split(":", 1)[1]
    ...
    if use_llm:
        summary = llm_chat(
            ...
            + "\nOFFICIAL FCG FACTS PER COUNTRY (cite these specifics — "
              "lead times, approvers, fees, hours — verbatim where useful):\n"
            + "\n".join(
                f"{code} ({role}):\n{facts}"
                for (code, role, _a) in verdicts
                for facts in [country_key_facts(code, fcg, role)]
                if facts)
            + ("\nCompliant alternative suggestions (user-requested airports "
               ...
               + json.dumps([{k: v for k, v in s.items()
                            ...
                            indent=1)
               if suggestions else "")
            + "\nWrite a detailed operational briefing (<=450 words): per-leg "
              "rationale; for EVERY overflight country state its specific "
              "clearance lead time, approver/channel, and fees from the FCG "
              "facts above (never say 'not detailed' if a fact is listed); "
              "for each landing, the airport choice rationale, customs and "
              "operating-hours specifics, and payment method; a consolidated "
              "lead-time action timeline (what to submit when, working "
              "backwards from the mission date); cautions; and if "
              "alternatives exist, a one-line recommendation on "
              "requested-vs-alternative airports.")
        print("\n=== BRIEFING ===\n" + summary)

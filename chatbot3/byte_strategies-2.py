...
STEP 2 — PARSING: What does that function do with the input?
  Does it parse a header? Check magic bytes? Read length fields?
  What format does it expect? (binary, text, protocol, file format)

STEP 3 — PATH TO BUG: How do bytes reach the vulnerable code?
...
STEP 4 — TRIGGER: What specific input values trigger the bug?
...
4. Use ONLY Python standard library (struct, sys, os, etc.)
5. Follow the analysis — craft bytes that satisfy each parsing step
   identified in the analysis to reach the vulnerable code path.
6. Include comments explaining what each part of the input does:
   # bytes 0-3: magic header expected by the parser
   # bytes 4-7: length field (set to 0xFFFFFFFF to trigger overflow)
   # etc.
7. If the analysis says the parser expects a specific format (ELF, MQTT,
   JSON, etc.), generate VALID-LOOKING input with the SPECIFIC FIELD
   malformed to trigger the bug.
8. Generate MULTIPLE variants if possible — output the best one.
"""
...
class DeepTraceStrategy(ByteStrategy):
    ...
    def generate(self, context, router, harness=None):
        ...
        trace_prompt = (ctx +
            ...
            "STEP 1 — ENTRY: How does the harness pass bytes to the project?\n"
            "STEP 2 — PARSING: What format/structure does the first function expect?\n"
            "STEP 3 — PATH TO BUG: What call chain leads to the vulnerable code?\n"
            "STEP 4 — TRIGGER: What specific byte values/structure triggers the bug?\n\n"
            "Be SPECIFIC. Name exact functions, struct fields, conditions, byte offsets.\n")

        print(f"  [{self.name}] Step 1: Tracing code path...")
        analysis = router.chat(system_prompt=SYSTEM_PROMPT_CODE_TRACER, user_prompt=trace_prompt, max_tokens=2048, temperature=0.1)
        if not analysis:
            return results
...
class DirectScriptStrategy(ByteStrategy):
    ...
    def generate(self, context, router, harness=None):
        ...
        prompt = (ctx +
            ...
            "1. The harness receives raw bytes and calls which function?\n"
            "2. That function expects what format/structure?\n"
            "3. What values in that structure trigger the bug?\n"
            "4. Write a script that constructs those exact bytes.\n\n"

...
Dual FAISS indexes over training data:
  1. Question-level index — few-shot example retrieval
  2. Snippet-level index  — context retrieval (supplements PubMed)
"""
...
def load_questions(filepath, is_training=False):
    ...
    for q in data["questions"]:
        bq = BioASQQuestion(id=q["id"], body=q["body"], qtype=q["type"])
...
class TrainingIndex:
    ...
    def __init__(self, questions, embedding_model_name):
        ...
        logger.info(f"Training corpus: {len(questions)} Qs, {len(self.all_snippets)} snippets")
    ...
    def build(self):
        try:
            ...
            logger.warning("faiss-cpu or sentence-transformers not installed")
            return
        ...
        q_embs = self._model.encode(q_texts, show_progress_bar=True,
                                     normalize_embeddings=True, batch_size=256).astype("float32")
        dim = q_embs.shape[1]
        import faiss
    ...
    def retrieve_similar_questions(self, question, top_k=3, same_type=True):
        ...
        scores, indices = self._q_index.search(query_emb, min(search_k, self._q_index.ntotal))

        results = []
        for idx in indices[0]:
            if idx < 0:
                continue
            c = self.questions[idx]
            if same_type and c.qtype != question.qtype:
                continue
            if c.id == question.id:
                continue
            results.append(c)
    ...
    def retrieve_snippets(self, question, top_k=10):
        if self._s_index is None or self._model is None:
            similar_qs = self.retrieve_similar_questions(question, top_k=5, same_type=False)
            snippets = []
            for q in similar_qs:
...
def format_submission(questions):
    out_questions = []
    for q in questions:
        entry = {"id": q.id, "type": q.qtype, "body": q.body}

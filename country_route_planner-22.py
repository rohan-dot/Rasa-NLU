...
AIRCRAFT_CSV = "aircraft_specs.csv"      # optional: type,range_nm,min_runway_ft,notes
AIRCRAFT = None                          # set by --aircraft: dict or {"type": name}
AUTO_STOPS = False                       # set by --auto-stops (needs aircraft range)
LANDING_PENALTY_NM = 400.0               # cost of one extra landing in auto-stop search
AUTO_STOP_DETOUR_FACTOR = 1.5            # candidate stops within this x direct distance
AUTO_STOP_SET = set()                    # airports inserted automatically (for reports)
AUTO_STOP_FALLBACK = True                # also consider OurAirports large airports
NON_DESIGNATED_PENALTY_NM = 600.0        # extra cost for a non-FCG-designated stop
AUTO_STOP_FALLBACK_TYPES = {"large_airport"}
...
# Exact overflight detection using Natural Earth borders, if the file is
# present (and shapely is installed). Otherwise the centroid-radius model
# below is used. Recommended: download ne_110m_admin_0_countries.geojson.
NE_FILE = "ne_110m_admin_0_countries.geojson"
POLYGON_SAMPLE_NM = 15.0        # track sampling step for polygon checks
_POLY = None                    # lazily loaded {iso3: geometry}
_POLY_TREE = None
CORRIDOR_RADIUS_NM    = 150.0   # a country is "overflown" if its centroid
...
STONE_MAX_HOP_NM      = 4500    # longest single hop between stones
STONE_BACKTRACK_NM    = 600     # allow stones this much 'backwards'
...
def load_fcg(path: str) -> pd.DataFrame:
    ...
    print(f"[load] FCG: {len(df)} country rows, {len(df.columns)} columns")
    # Audit: which configured fields the judge/fuel note will actually see
    present = [c for c in FEASIBILITY_COLS if c in df.columns]
    missing = [c for c in FEASIBILITY_COLS if c not in df.columns]
    print(f"[load] judge reads {len(present)}/{len(FEASIBILITY_COLS)} "
          f"configured fields")
    if missing:
        print(f"[load] WARNING judge fields NOT in this extract: {missing}")
        unused = [c for c in df.columns
                  if c not in FEASIBILITY_COLS and c not in PAYMENT_COLS
                  and c not in (FCG_CODE_COL, "country", "country_name",
                                "alpha2", "alpha3")]
        print(f"[load] columns in extract the judge does NOT read: {unused}")
    pay_missing = [c for c in PAYMENT_COLS if c not in df.columns]
    if pay_missing:
        print(f"[load] WARNING payment fields NOT in this extract: {pay_missing}")
    return df
...
def build_airport_index(fcg: pd.DataFrame, ourairports: pd.DataFrame):
    ...
    extract_cols = [c for c in (AIRPORT_COL, AIRPORT_COL_2)
                    if c in fcg.columns]
    if not extract_cols:
        # Auto-detect: any column whose name looks like entry/exit/airfield
        pat = re.compile(r"(entry|exit|airfield|airport|alternate|aerodrome)",
                         re.IGNORECASE)
        extract_cols = [c for c in fcg.columns
                        if pat.search(c) and c != FCG_CODE_COL]
        if extract_cols:
            print(f"[airports] configured columns missing; auto-detected "
                  f"{extract_cols}")
        else:
            print(f"[airports] no entry/exit/airfield column found in "
                  f"{list(fcg.columns)[:12]}... — airport features off. "
                  f"Set AIRPORT_COL in CONFIG to the right column name.")
            return {}, {}
    ...
    for _, row in fcg.iterrows():
        ...
        for tok in AIRPORT_CODE_RE.findall(text.upper()):
            ...
            if tok.isdigit():
                continue
            # Only pure-alphabetic tokens can be ICAO (4) / IATA (3) codes.
            # Aircraft types (MD11, A340, B777, AN24), runway designators
            # (02R, 13L) and units (15KT, 49FT) all contain digits.
            if not tok.isalpha():
                continue
...
def load_polygons():
    """Load Natural Earth admin-0 polygons once. Returns (dict, tree) or
    (None, None) if unavailable."""
    global _POLY, _POLY_TREE
    if _POLY is not None:
        return _POLY, _POLY_TREE
    try:
        from shapely.geometry import shape
        from shapely.strtree import STRtree
        gj = json.load(open(NE_FILE, encoding="utf-8"))
    except Exception as e:
        _POLY, _POLY_TREE = {}, None
        print(f"[geom] polygons unavailable ({e.__class__.__name__}); using "
              f"centroid-radius approximation")
        return _POLY, _POLY_TREE
    polys, geoms, codes = {}, [], []
    for f in gj["features"]:
        pr = f["properties"]
        iso = str(pr.get("ISO_A3", "-99"))
        if iso in ("-99", "", "None"):
            iso = str(pr.get("ADM0_A3", "-99"))
        if iso == "-99":
            continue
        g = shape(f["geometry"])
        polys[iso.upper()] = g
        geoms.append(g)
        codes.append(iso.upper())
    _POLY = polys
    # shapely 2.x STRtree.query returns indices; 1.8 returns geometries
    _POLY_TREE = (STRtree(geoms), codes, {id(g): c for g, c in zip(geoms, codes)})
    print(f"[geom] exact overflight detection: {len(polys)} country borders")
    return _POLY, _POLY_TREE


def direct_corridor(p1, p2, nodes, endpoints):
    """Countries the direct great-circle track crosses.

    Exact (polygon) mode when Natural Earth borders are available: a country
    is overflown iff a sampled track point lies inside its border.
    Fallback: centroid-radius approximation. Endpoint countries excluded
    from the clearance count but included in the corridor list."""
    polys, tree = load_polygons()
    if polys:
        from shapely.geometry import Point
        samples, total_nm = gc_sample(p1, p2, step_nm=POLYGON_SAMPLE_NM)
        strtree, codes, by_id = tree
        near = {}
        for idx, (lat, lon) in enumerate(samples):
            pt = Point(lon, lat)
            for hit in strtree.query(pt):
                code = codes[int(hit)] if isinstance(hit, (int,)) or \
                    hasattr(hit, "__index__") else by_id.get(id(hit))
                if code is None:
                    continue
                if code in near or code not in nodes.index:
                    continue
                if polys[code].contains(pt):
                    near[code] = idx
        ordered = sorted(near, key=near.get)
        clearances = [c for c in ordered if c not in endpoints]
        return [endpoints[0]] + clearances + [endpoints[1]], clearances, total_nm
...
def stepping_stone_corridors(p_src, p_dst, src, dst, nodes, blocked,
    ...
    Nodes: 'SRC' (origin airport point), every non-blocked country centroid,
    'DST' (destination airport point). An edge u->v is the great-circle hop
    between them; it exists only if the hop overflies no blocked country and
    is <= STONE_MAX_HOP_NM. Edge cost = hop NM + clearance cost of every
    country the hop overflies (+ the stone itself). Hop corridors are
    computed lazily and cached, so only hops the search actually considers
    are evaluated.

    Returns list of (score, label, corridor_countries, total_nm, clearances)
    — best path first, followed by a few 1-stone alternates for display."""
    cent = {c: (float(nodes.loc[c, "lat"]), float(nodes.loc[c, "lon"]))
            for c in nodes.index if c not in blocked}
    pos = dict(cent)
    pos["SRC"], pos["DST"] = p_src, p_dst
    # forward-progress filter: only consider stones within the detour ellipse
    stones = [c for c, pc in cent.items()
              if c not in (src, dst)
              and haversine_nm(*p_src, *pc) + haversine_nm(*pc, *p_dst)
              <= STEPPING_DETOUR_FACTOR * direct_nm]
    code_of = {"SRC": src, "DST": dst}

    hop_cache = {}
    def hop(u, v):
        key = (u, v)
        if key not in hop_cache:
            a, b = code_of.get(u, u), code_of.get(v, v)
            _, clear, nm = direct_corridor(pos[u], pos[v], nodes, (a, b))
            clear = [c for c in clear if c not in (src, dst)]
            ok = not (set(clear) & blocked)
            hop_cache[key] = (clear, nm, ok)
    ...
    pq = [(0.0, "SRC")]
    seen = set()
    d_to_dst = {c: haversine_nm(*cent[c], *p_dst) for c in stones}
    while pq:
        d, u = heapq.heappop(pq)
        if u in seen:
            continue
        ...
        if u == "DST":
            break
        # neighbours: DST always; stones that make forward progress
        u_to_dst = haversine_nm(*pos[u], *p_dst)
        neigh = ["DST"] + [c for c in stones
                           if c not in seen and c != u
                           and d_to_dst[c] <= u_to_dst + STONE_BACKTRACK_NM]
        for v in neigh:
            hop_nm = haversine_nm(*pos[u], *pos[v])
            if v != "DST" and hop_nm > STONE_MAX_HOP_NM:
                continue
            clear, nm, ok = hop(u, v)
            if not ok:
                continue
            step = nm + sum(CLEARANCE_PENALTY_NM + RISK_EXTRA.get(c, 0.0)
                            for c in clear)
            if v != "DST":
                step += CLEARANCE_PENALTY_NM + RISK_EXTRA.get(v, 0.0)
            nd = d + step
    ...
    if "DST" in dist:
        ...
        while path[-1] != "SRC":
            path.append(prev[path[-1]])
        path.reverse()
        # assemble along-track corridor and true NM
        seen_c, clear_all, total = set(), [], 0.0
        for u, v in zip(path, path[1:]):
            clear, nm, _ = hop(u, v)
            total += nm
            for c in clear + ([v] if v not in ("SRC", "DST") else []):
                if c not in seen_c and c not in (src, dst):
                    seen_c.add(c)
                    clear_all.append(c)
        stones_used = [s for s in path if s not in ("SRC", "DST")]
        label = ("via " + " -> ".join(stones_used)) if stones_used else "DIRECT"
        out.append((corridor_score(total, clear_all), label,
                    [src] + clear_all + [dst], total, clear_all))
    # a few 1-stone alternates for the report
    alts = []
    for x in stones:
        c1, n1, ok1 = hop("SRC", x)
        c2, n2, ok2 = hop(x, "DST")
        ...
        for c in c1 + [x] + c2:
            if c not in seen_c and c not in (src, dst):
                ...
                clear.append(c)
        nm = n1 + n2
        alts.append((corridor_score(nm, clear), f"via {x}",
...
def judge_country(code, name, role, travel_date, fcg, airport=None,
    ...
    if VERBOSE:
        ...
        print(dossier)
        print("----- END DOSSIER -----\n")
    # Days available for clearance processing
    import datetime as _dt
    try:
        mission = _dt.date.fromisoformat(str(travel_date))
        days_until = (mission - _dt.date.today()).days
    except ValueError:
        days_until = None
    days_txt = (f"Today is {_dt.date.today().isoformat()}; the mission is "
                f"{days_until} days away." if days_until is not None else "")
    ac_txt = ""
    if AIRCRAFT:
        ac_txt = (f"Aircraft: {AIRCRAFT['type']}"
                  + (f" (needs ~{AIRCRAFT['min_runway_ft']:,.0f} ft runway)"
                     if AIRCRAFT.get("min_runway_ft") else "")
                  + ". Apply any aircraft-type-specific rules in the data "
                    "(armed/combat aircraft restrictions, C-17/ER "
                    "requirements, weight bearing/runway limits, MOG).\n")
    ans = llm_json(
        f"Planned mission date: {travel_date}. {days_txt}\n{ac_txt}"
        ...
        f"this role? Consider overflight permission, entry/exit and customs "
        f"rules if landing, and lead times. Extract the MINIMUM lead time in "
        f"days required for this role (business days ~ x1.4 calendar days; "
        f"null if none stated). If data is missing or ambiguous, allow but "
        f"flag caution.\n"
        f'Respond ONLY JSON: {{"allowed": true/false, "caution": true/false, '
        f'"airport_ok": true/false, "min_lead_days": <int or null>, '
    ...
    # Deterministic lead-time enforcement (the LLM only extracts the number)
    lead = ans.get("min_lead_days")
    if isinstance(lead, (int, float)) and days_until is not None \
            and lead > days_until:
        msg = (f"LEAD TIME NOT MET: requires {int(lead)} days, only "
               f"{days_until} available. ")
        if role == "overflight":
            ans["allowed"] = False
        ans["caution"] = True
        ans["reason"] = msg + str(ans.get("reason", ""))
    return ans
...
def load_aircraft(type_name, path=AIRCRAFT_CSV):
    """Resolve --aircraft to a spec dict. Unknown types still work (judge
    context only); known types also constrain range."""
    if not type_name:
        return None
    want = type_name.strip().upper().replace("-", "").replace(" ", "")
    try:
        df = pd.read_csv(path)
        for _, r in df.iterrows():
            have = str(r["type"]).strip().upper().replace("-", "").replace(" ", "")
            if have == want:
                spec = {"type": str(r["type"]), "range_nm": float(r["range_nm"]),
                        "min_runway_ft": float(r["min_runway_ft"]),
                        "notes": str(r.get("notes", ""))}
                print(f"[aircraft] {spec['type']}: range {spec['range_nm']:,.0f} NM, "
                      f"min runway {spec['min_runway_ft']:,.0f} ft")
                return spec
    except FileNotFoundError:
        pass
    print(f"[aircraft] '{type_name}' not in {path} — no range enforcement; "
          f"type still passed to the clearance judge")
...
def auto_stop_search(p_src, src_ctry, src_ap, p_dst, dst_ctry, dst_ap,
                     candidates, ourairports, nodes, blocked, banned,
                     range_nm):
    """Dijkstra over FCG-designated airports to insert fuel/staging stops so
    that no leg exceeds 85% of aircraft range.

    Edge u->v exists if leg <= cap and its overflight corridor crosses no
    blocked country. Edge cost = leg NM + clearance cost of overflown
    countries + (for a landing) CLEARANCE_PENALTY for the stop country +
    LANDING_PENALTY_NM. Returns ordered list of inserted airport codes
    (possibly empty if the direct leg fits), or None if no chain exists."""
    cap = 0.85 * range_nm
    direct = haversine_nm(*p_src, *p_dst)
    if direct <= cap:
        return []
    # candidate pool: designated airports with coords, inside detour ellipse
    pool = {}
    for ctry, aps in candidates.items():
        for ap in aps:
            if ap in banned or ap in (src_ap, dst_ap):
                continue
            if ctry in blocked:
                continue          # never land in a blocked country
            p = airport_pt(ap, ourairports)
            if haversine_nm(*p_src, *p) + haversine_nm(*p, *p_dst) \
                    <= AUTO_STOP_DETOUR_FACTOR * direct:
                pool[ap] = (p, ctry)
    n_designated = len(pool)
    fallback = set()
    if AUTO_STOP_FALLBACK and "type" in ourairports.columns:
        big = ourairports[ourairports["type"].isin(AUTO_STOP_FALLBACK_TYPES)]
        for ap, r in big.iterrows():
            if ap in pool or ap in banned or ap in (src_ap, dst_ap):
                continue
            ctry = str(r["iso3"])
            if ctry in blocked or ctry not in nodes.index:
                continue
            ...
            if haversine_nm(*p_src, *p) + haversine_nm(*p, *p_dst) \
                ...
                pool[ap] = (p, ctry)
                fallback.add(ap)
    pos = {ap: v[0] for ap, v in pool.items()}
    ctry_of = {ap: v[1] for ap, v in pool.items()}
    pos["SRC"], pos["DST"] = p_src, p_dst
    ctry_of["SRC"], ctry_of["DST"] = src_ctry, dst_ctry
    print(f"[auto-stops] {src_ap or src_ctry}->{dst_ap or dst_ctry}: direct "
          f"{direct:,.0f} NM > cap {cap:,.0f} NM; {n_designated} FCG-designated "
          f"candidate stops + {len(fallback)} large-airport fallbacks "
          f"(+{NON_DESIGNATED_PENALTY_NM:.0f} NM each)")

    hop_cache = {}
    def hop(u, v):
        key = (u, v)
        if key not in hop_cache:
            _, clear, nm = direct_corridor(pos[u], pos[v], nodes,
                                           (ctry_of[u], ctry_of[v]))
            ok = not (set(clear) & blocked)
            hop_cache[key] = (clear, nm, ok)
        return hop_cache[key]
    ...
    INF = float("inf")
    dist, prev, pq, seen = {"SRC": 0.0}, {}, [(0.0, "SRC")], set()
    d_to_dst = {ap: haversine_nm(*pos[ap], *p_dst) for ap in pool}
    while pq:
        d, u = heapq.heappop(pq)
        if u in seen:
            continue
        ...
        if u == "DST":
            break
        u_to_dst = haversine_nm(*pos[u], *p_dst)
        neigh = ["DST"] + [ap for ap in pool
                           if ap not in seen and ap != u
                           and d_to_dst[ap] <= u_to_dst + STONE_BACKTRACK_NM]
        for v in neigh:
            leg = haversine_nm(*pos[u], *pos[v])
            if leg > cap:
                continue
            clear, nm, ok = hop(u, v)
            if not ok:
            ...
            step = nm + sum(CLEARANCE_PENALTY_NM + RISK_EXTRA.get(c, 0.0)
                            for c in clear)
            if v != "DST":
                step += (CLEARANCE_PENALTY_NM + RISK_EXTRA.get(ctry_of[v], 0.0)
                         + LANDING_PENALTY_NM)
                if v in fallback:
                    step += NON_DESIGNATED_PENALTY_NM
            nd = d + step
            if nd < dist.get(v, INF):
                dist[v], prev[v] = nd, u
                heapq.heappush(pq, (nd, v))
    ...
    while path[-1] != "SRC":
        path.append(prev[path[-1]])
    path.reverse()
    stops = [s for s in path if s not in ("SRC", "DST")]
    print("[auto-stops] inserted: " + (" -> ".join(
        s + (" (non-designated, verify)" if s in fallback else "")
        for s in stops) or "(none)"))
    return stops


def expand_auto_stops(stop_countries, pinned, candidates, ourairports, nodes,
                      blocked, banned, range_nm):
    """Insert auto stops between each pair of user waypoints. Returns new
    (stop_countries, pinned, auto_set)."""
    def point(i):
        if i in pinned and pinned[i] in ourairports.index:
            return airport_pt(pinned[i], ourairports), pinned[i]
        c = stop_countries[i]
        return (float(nodes.loc[c, "lat"]), float(nodes.loc[c, "lon"])), None

    new_c, new_p, auto = [], {}, set()
    for i in range(len(stop_countries) - 1):
        pa, apa = point(i)
        pb, apb = point(i + 1)
        new_p_idx = len(new_c)
        new_c.append(stop_countries[i])
        if i in pinned:
            new_p[new_p_idx] = pinned[i]
        inserted = auto_stop_search(pa, stop_countries[i], apa, pb,
                                    stop_countries[i + 1], apb, candidates,
                                    ourairports, nodes, blocked, banned,
                                    range_nm)
        if inserted is None:
            sys.exit(f"[FATAL] --auto-stops: no chain of designated airports "
                     f"connects {apa or stop_countries[i]} to "
                     f"{apb or stop_countries[i+1]} within "
                     f"{0.85*range_nm:,.0f} NM legs avoiding {sorted(blocked)}.")
        for ap in inserted:
            new_p[len(new_c)] = ap
            new_c.append(str(ourairports.loc[ap, "iso3"]))
            auto.add(ap)
    last = len(stop_countries) - 1
    new_p_idx = len(new_c)
    new_c.append(stop_countries[last])
    if last in pinned:
        new_p[new_p_idx] = pinned[last]
    return new_c, new_p, auto
...
def plan(query, travel_date, use_llm):
    ...
    user_stops, user_pinned = list(stop_countries), dict(pinned)
    if AUTO_STOPS and not (AIRCRAFT and AIRCRAFT.get("range_nm")):
        sys.exit("[FATAL] --auto-stops needs --aircraft <type> with a range "
                 "in aircraft_specs.csv")
    ...
    global STONE_MAX_HOP_NM, MAX_LEG_NM
    if AIRCRAFT and AIRCRAFT.get("range_nm"):
        cap = 0.85 * AIRCRAFT["range_nm"]   # 15% fuel reserve margin
        if cap < STONE_MAX_HOP_NM:
            STONE_MAX_HOP_NM = cap
            print(f"[aircraft] hop length capped at {cap:,.0f} NM "
                  f"(85% of {AIRCRAFT['type']} range)")
    ...
    global AUTO_STOP_SET
    for outer in range(1, 11):
        if AUTO_STOPS:
            stop_countries, pinned, AUTO_STOP_SET = expand_auto_stops(
                user_stops, user_pinned, candidates, ourairports, nodes,
                blocked, banned_airports, AIRCRAFT["range_nm"])
            print("[route] with auto stops: " + " -> ".join(
                f"{c}({pinned[i]})" if i in pinned else c
                for i, c in enumerate(stop_countries)))
        fuel_notes = make_fuel_notes(stop_countries, fcg, fuel_rates)
        ...
        for i, (a, b) in enumerate(zip(stop_countries, stop_countries[1:])):
            ...
            if not cands:
                sys.exit(f"[FATAL] no corridor {a}->{b}: direct crosses "
                         ...
                         f"        Many of these are LEAD TIME denials — try a "
                         f"later --date, or add an intermediate stop as a "
                         f"waypoint (e.g. '... to rjtt to ... to {b}').")
            cands.sort(key=lambda t: t[0])
            best = cands[0]
            corridors.append(best[2])
            corridor_log.append({
                "leg": f"{chosen[i]} -> {chosen[i + 1]}",
                "countries": f"{a} -> {b}",
                "chosen": {"option": best[1], "nm": round(best[3]),
                           "clearances": list(best[4]),
                           "score": round(best[0])},
                "alternatives": [{"option": c_[1], "nm": round(c_[3]),
                                  "clearances": list(c_[4]),
                                  "score": round(c_[0])} for c_ in cands[1:5]],
                "direct_rejected_blocked": d_blocked})
        ...
        for i, (ctry, ap) in enumerate(zip(stop_countries, chosen)):
            ...
            if key not in verdicts:
                ...
                if ap in pinned_set and ap not in AUTO_STOP_SET \
                        and ap not in airport_to_country and \
                ...
                if not v.get("airport_ok", True) \
                        ...
                        and (ap not in pinned_set or ap in AUTO_STOP_SET):
                    banned_airports.add(ap)
                    changed = True
                    if ap in AUTO_STOP_SET:
                        print(f"    auto stop {ap} rejected — re-searching")
        ...
        for path in corridors:
            for code in path:
                ...
                if not v["allowed"] and code not in blocked:
                    ...
                    changed = True
        if not changed:
            # Export FCG-derived constraints so eval_routes.py can replay
            # the routing under exactly these blocks (--blocks-file).
            try:
                json.dump({"query": query, "date": travel_date,
                           "blocked_countries": sorted(blocked),
                           "banned_airports": sorted(banned_airports),
                           "stops": list(chosen),
                           "stop_countries": list(stop_countries),
                           "auto_stops": sorted(AUTO_STOP_SET),
                           "aircraft": AIRCRAFT,
                           "corridors": corridor_log,
                           "total_nm": round(total_nm),
                           "fuel_notes": fuel_notes,
                           "config": {"clearance_penalty_nm": CLEARANCE_PENALTY_NM,
                                      "landing_penalty_nm": LANDING_PENALTY_NM,
                                      "risk_avoid_penalty_nm": RISK_AVOID_PENALTY_NM,
                                      "non_designated_penalty_nm": NON_DESIGNATED_PENALTY_NM,
                                      "range_fraction": 0.85},
                           "verdicts": {f"{c}/{r}/{a}": v
                                        for (c, r, a), v in verdicts.items()}},
                          open("last_run_blocks.json", "w"), indent=1)
                print("[export] FCG-derived blocks -> last_run_blocks.json")
            except Exception as e:
                print(f"[export] could not write last_run_blocks.json: {e}")
...
def narrate(chosen, total_nm, alternates, stop_countries, corridors,
    ...
    for i, (ctry, ap) in enumerate(zip(stop_countries, chosen)):
        print(f"Stop {i + 1}: {nodes.loc[ctry, 'country']} ({ctry}) — "
              f"{ap_name(ap)}"
              + ("  [AUTO fuel/staging stop]" if ap in AUTO_STOP_SET else ""))
        if fuel_notes:
    ...
    for i, ((a, b), corridor) in enumerate(
        ...
        print(f"  {a} -> {b}: {d:,.0f} NM | overflight corridor: "
              f"{' -> '.join(corridor)}")
        if AIRCRAFT and AIRCRAFT.get("range_nm") \
                and d > 0.85 * AIRCRAFT["range_nm"]:
            print(f"      WARNING: leg exceeds 85% of {AIRCRAFT['type']} "
                  f"range ({AIRCRAFT['range_nm']:,.0f} NM) — add a fuel-stop "
                  f"waypoint to this leg")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--aircraft", default=None,
                    help="aircraft type (e.g. C-17); constrains hop range "
                         "and informs clearance judging")
    ap.add_argument("--auto-stops", action="store_true",
                    help="insert fuel/staging stops automatically so no leg "
                         "exceeds 85%% of aircraft range (needs --aircraft)")
    ...
    try:
        ...
        sys.exit(f"[FATAL] --date '{args.date}' is not a valid YYYY-MM-DD "
                 f"date. Lead-time checks depend on it.")
    ...
    if args.risk:
        USE_RISK_FEED = True
    if args.aircraft:
        AIRCRAFT = load_aircraft(args.aircraft)
    if args.auto_stops:
        AUTO_STOPS = True
    if args.list:

...
FCG_CSV      = _env("FCG_CSV", default="fcg_extract.csv")
MAPPING_CSV  = _env("FCG_MAPPING_CSV", default="mapping.csv")
COORDS_CSV   = _env("FCG_COORDS_CSV",
                    default="countries_codes_and_coordinates.csv")
AIRPORTS_CSV = _env("FCG_AIRPORTS_CSV", default="airports.csv")  # OurAirports
FUEL_CSV     = _env("FCG_FUEL_CSV", default="fuel_rates.csv")    # optional
# Which DLA fuel codes to show in the per-stop fuel note
FUEL_CODES_SHOWN = ["JP8", "JAA", "IA1", "NA1"]

# LLM endpoint: honors the LITELLM_*/AGENT_* env vars (Claude via LiteLLM
# gateway) and falls back to the local vLLM/Gemma defaults when unset, so
# the same script runs in both environments without edits.
LLM_BASE     = _env("FCG_LLM_BASE_URL", "LITELLM_BASE_URL",
                    default="http://localhost:8000/v1").rstrip("/")
LLM_URL      = LLM_BASE + "/chat/completions"
LLM_MODEL    = _env("FCG_LLM_MODEL", "AGENT_MODEL", default="gemma-4-31B-it")
LLM_API_KEY  = _env("FCG_LLM_API_KEY", "LITELLM_API_KEY", default="")
LLM_TLS_VERIFY = _env("FCG_TLS_VERIFY", "AGENT_TLS_VERIFY",
...
FCG_CODE_COL = "airport"        # auto-detected in load_fcg ('airport'|'country')
AIRPORT_COL  = "entry_exit_airports_summary"
# The judge reads the RAW extracted text (source of truth), not summaries.
# Baseline column set; load_fcg() extends this DYNAMICALLY with every column
# in the CSV whose name ends in _raw, so new checklist-driven fields flow
# into the dossier with zero code changes.
FEASIBILITY_COLS = [
    "overflight_raw",
...
DOSSIER_CHAR_CAP = int(_env("FCG_DOSSIER_CHAR_CAP", default="6000"))
if not LLM_TLS_VERIFY:
    import urllib3
...
def load_fcg(path: str) -> pd.DataFrame:
    global FCG_CODE_COL, FEASIBILITY_COLS
    df = pd.read_csv(path)
    # Code column: 'airport' (legacy extract) or 'country' (pipeline extract)
    if FCG_CODE_COL not in df.columns:
        if "country" in df.columns:
            FCG_CODE_COL = "country"
        else:
            sys.exit(f"[FATAL] FCG CSV lacks 'airport'/'country' code column; "
                     f"found {list(df.columns)}")
    df[FCG_CODE_COL] = (df[FCG_CODE_COL].astype(str)
                        .str.strip().str.upper()
                        .str.replace(r"_FCG.*$", "", regex=True))
    # Treat literal 'NA' extraction answers as missing so dossiers and the
    # airport index skip them cleanly.
    raw_cols = [c for c in df.columns if c.endswith("_raw")
                or c == AIRPORT_COL]
    for c in raw_cols:
        df[c] = df[c].replace(r"^\s*NA\s*$", pd.NA, regex=True)
    # Dynamic dossier: every *_raw column in the CSV, baseline order first,
    # then any new checklist-driven fields in CSV order.
    dynamic = [c for c in df.columns
               if c.endswith("_raw") and c not in FEASIBILITY_COLS]
    FEASIBILITY_COLS = ([c for c in FEASIBILITY_COLS if c in df.columns]
                        + dynamic)
    print(f"[load] FCG: {len(df)} country rows (code col '{FCG_CODE_COL}'; "
          f"dossier cols: {len(FEASIBILITY_COLS)})")
    if dynamic:
        print(f"[load] extra checklist-driven dossier fields: {dynamic}")
    return df
...
def build_airport_index(fcg: pd.DataFrame, ourairports: pd.DataFrame):
    ...
    extract_cols = [c for c in (AIRPORT_COL, "entry_exit_airports_raw")
                    if c in fcg.columns]
    if not extract_cols:
        print(f"[airports] neither '{AIRPORT_COL}' nor "
              f"'entry_exit_airports_raw' present — airport features off")
        return {}, {}
...
def direct_corridor(p1, p2, nodes, endpoints):
    ...
    for c in nodes.index:
        ...
        for (slat, slon) in samples:
            if haversine_nm(clat, clon, slat, slon) <= CORRIDOR_RADIUS_NM:
                near.add(c)
                break
    clearances = near - set(endpoints)
    return sorted(near), sorted(clearances), total_nm
...
def llm_chat(prompt, system="You are a precise flight-clearance analyst. "
    ...
    r = requests.post(LLM_URL, json=payload, timeout=LLM_TIMEOUT,
                      headers=headers, verify=LLM_TLS_VERIFY)
    r.raise_for_status()
...
def plan(query, travel_date, use_llm):
    ...
    for outer in range(1, 11):
        ...
        for i, (a, b) in enumerate(zip(stop_countries, stop_countries[1:])):
            ...
            d_corridor, d_clear, d_nm = direct_corridor(
                stop_pt(chosen[i]), stop_pt(chosen[i + 1]), nodes, (a, b))
            d_blocked = [c for c in d_clear if c in blocked]
            d_score = (d_nm + CLEARANCE_PENALTY_NM * len(d_clear)
                       if not d_blocked else float("inf"))
            # Option 2: land route through the country graph
            l_path, l_nm = dijkstra(graph, a, b, blocked)
            l_clear = [c for c in (l_path or []) if c not in (a, b)]
            l_score = (l_nm + CLEARANCE_PENALTY_NM * len(l_clear)
                       if l_path else float("inf"))
            if d_score == float("inf") and l_score == float("inf"):
                sys.exit(f"[FATAL] no corridor {a}->{b}: direct crosses "
                         f"blocked {d_blocked}, land route impossible with "
                         f"{sorted(blocked)} blocked.")
            if d_score <= l_score:
                corridors.append(d_corridor)
                clear_txt = ", ".join(d_clear) if d_clear \
                    else "none - international waters"
                print(f"    [corridor {a}->{b}] DIRECT/oceanic chosen: "
                      f"{d_nm:,.0f} NM, {len(d_clear)} clearances "
                      f"({clear_txt}) | land alt: "
                      + (f"{l_nm:,.0f} NM, {len(l_clear)} clearances"
                         if l_path else "none"))
            else:
                corridors.append(l_path)
                print(f"    [corridor {a}->{b}] LAND route chosen: "
                      f"{l_nm:,.0f} NM, {len(l_clear)} clearances | direct "
                      f"alt: {d_nm:,.0f} NM, {len(d_clear)} clearances"
                      + (f" (crosses blocked {d_blocked})" if d_blocked
                         else ""))
        ...
        summary = llm_chat(
            ...
            + ("\nCompliant alternative suggestions (user-requested airports "
               ...
               + json.dumps([{k: v for k, v in s.items()
                            ...
                            indent=1)
               if suggestions else "")
            + "\nWrite a short operational briefing (<=220 words): per-leg "

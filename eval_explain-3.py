...
def explain_endpoints(facts, lines):
    s = load_csv("endpoints_compare.csv")
    if s is None or s.empty:
        return
    legs = load_csv("endpoints_legs.csv")
    ref = s[s.who == "reference"].iloc[0]
    pl = s[s.who == "planner"].iloc[0]
    lines.append("\n== ENDPOINTS-ONLY TEST (endpoints_compare.png) ==")
    lines.append("The planner was given ONLY the first and last stop plus the "
                 "aircraft, and chose its own fuel/staging stops. Compared to "
                 "the human's stops:")
    lines.append(f"  reference: {ref.stops}  -> {ref.total_nm:,.0f} NM, "
                 f"{ref.landings} landings, {ref.clearances} clearances, "
                 f"score {ref.score:,.0f}")
    lines.append(f"  planner:   {pl.stops}  -> {pl.total_nm:,.0f} NM, "
                 f"{pl.landings} landings, {pl.clearances} clearances, "
                 f"score {pl.score:,.0f}")
    if ref.legs_over_range:
        lines.append(f"  NOTE: {ref.legs_over_range} reference leg(s) exceed 85% "
                     f"of the aircraft's range — the human route may assume a "
                     f"different aircraft or air refueling.")
    if pl.score < ref.score:
        lines.append(f"  DECISION: PLANNER STOP SELECTION IS BETTER by "
                     f"{ref.score-pl.score:,.0f} points "
                     f"({pl.total_nm-ref.total_nm:+,.0f} NM, "
                     f"{pl.landings-ref.landings:+d} landings, "
                     f"{pl.clearances-ref.clearances:+d} clearances).")
        dec = "planner"
    elif pl.score > ref.score:
        lines.append(f"  DECISION: HUMAN STOP SELECTION IS BETTER by "
                     f"{pl.score-ref.score:,.0f} points "
                     f"({pl.total_nm-ref.total_nm:+,.0f} NM, "
                     f"{pl.landings-ref.landings:+d} landings, "
                     f"{pl.clearances-ref.clearances:+d} clearances). The "
                     f"human likely used knowledge the planner lacks (fuel "
                     f"availability, MOG, relationships) — worth encoding.")
        dec = "reference"
    else:
        lines.append("  DECISION: TIE.")
        dec = "tie"
    if legs is not None and len(legs):
        for who in ("reference", "planner"):
            lines.append(f"  {who} legs: " + "; ".join(
                f"{r.leg} {r.nm:,.0f} NM over [{r.overflights or 'water'}]"
                for _, r in legs[legs.who == who].iterrows()))
    lines.append("  Scoring: NM + 300 per clearance + 400 per extra landing.")
    facts["endpoints"] = {"decision": dec, "ref": ref.to_dict(), "planner": pl.to_dict()}

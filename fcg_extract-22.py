...
BATCH_SIZE = 20
RETRIES = 4
GUIDED = os.environ.get("FCG_GUIDED", "1").lower() not in ("0", "false", "no")
CHUNK_SIZE = int(os.environ.get("FCG_CHUNK_SIZE", "8000"))
CHUNK_OVERLAP = int(os.environ.get("FCG_CHUNK_OVERLAP", "800"))
HTTP_TIMEOUT = int(os.environ.get("FCG_TIMEOUT", "1200"))
...
SYSTEM_PROMPT = (
    ...
    "dossier does cover. If the closest passage does not genuinely address "
    "the field, use \"NA\" rather than quoting loosely related text. Respond "
    "with the literal string \"NA\" ONLY when "
...
# ----------------------------------------------------------------------------
# Checklist loading — items may be plain strings (legacy) or dicts:
#   {"id": ..., "question": ..., "type": "extract"|"workflow",
#    "scope": [key candidates], "route": [key candidates], "list": true}
# route/scope candidates: "name$" = exact key match, plain = substring.
# "workflow" items are mission-planner actions with no document answer:
# they keep their CSV column but are never sent to the LLM (src=WORKFLOW).
# ----------------------------------------------------------------------------
def load_checklist(path: Path):
    data = json.loads(path.read_text(encoding="utf-8"))
    if isinstance(data, dict):
        data = [{"id": k, "question": v} for k, v in data.items()]
    if not isinstance(data, list):
        die("Checklist JSON must be a list or object.")
    items = []
    for i, it in enumerate(data):
        if isinstance(it, str):
            items.append({"col": sanitize_col(it), "question": it,
                          "type": "extract", "scope": [], "route": [], "list": False})
            continue
        if not isinstance(it, dict):
            die(f"Checklist item {i} has unsupported type {type(it)}")
        name = None
        for k in ("id", "name", "item", "field", "title", "checklist_item"):
            if it.get(k):
                name = str(it[k]); break
        q = None
        ...
        items.append({
            "col": sanitize_col(name or q),
            "question": q or name,
            "type": (it.get("type") or "extract").lower(),
            "scope": list(it.get("scope") or []),
            "route": list(it.get("route") or []),
            "list": bool(it.get("list")),
        })
    seen = {}
    for it in items:
        n = seen.get(it["col"], 0); seen[it["col"]] = n + 1
        if n:
            it["col"] = f"{it['col']}_{n+1}"
    return items
...
# ----------------------------------------------------------------------------
# Key-name routing (from the JSON-native design): each item is directed to the
# subtree(s) of the FCG doc whose key names match its route candidates,
# optionally only inside matching scope subtrees. Items with no route hits
# fall back to the full flattened dossier.
# ----------------------------------------------------------------------------
def _key_match(key: str, cand: str) -> bool:
    k = key.lower()
    return k == cand[:-1].lower() if cand.endswith("$") else cand.lower() in k


def find_subtrees(obj, candidates, path="$"):
    """(path, subtree) for every value whose KEY matches a candidate.
    Does not descend into an already-matched subtree."""
    hits = []
    ...
    def walk(node, p):
        if isinstance(node, dict):
            for k, v in node.items():
                if META_KEY.search(k):
                    continue  # never route into *Path metadata keys
                kp = f"{p}.{k}"
                if any(_key_match(k, c) for c in candidates) and v not in (None, "", [], {}):
                    hits.append((kp, v))
                else:
                    walk(v, kp)
...
def flatten_subtree(obj, prefix="") -> str:
    lines = []
    ...
    def walk(o, pre):
        if isinstance(o, dict):
            for k, v in o.items():
                if META_KEY.search(k):
                    continue
                walk(v, f"{pre}{k}." if pre else f"{k}.")
        elif isinstance(o, list):
            for i, v in enumerate(o):
                walk(v, f"{pre}{i}.")
        else:
            if o is not None and str(o).strip():
                t = html_to_text(str(o))
                if t:
                    lines.append(f"{pre.rstrip('.')}: {t}")

    walk(obj, prefix)
    return "\n".join(lines)


def route_text(doc, item) -> str:
    """Focused text for one routed item; '' when no route hits."""
    if not item.get("route"):
        return ""
    roots = [("$", doc)]
    if item.get("scope"):
        scoped = find_subtrees(doc, item["scope"])
        if scoped:
            roots = scoped
    parts = []
    for rp, root in roots:
        for p, sub in find_subtrees(root, item["route"], path=rp):
            body = flatten_subtree(sub) if isinstance(sub, (dict, list)) else html_to_text(str(sub))
            if body.strip():
                parts.append(f"### {p}\n{body}")
    return "\n\n".join(parts)
...
def llm_extract(dossier: str, fields):
    ...
    def fline(it):
        extra = " (COMPLETE list — copy EVERY matching entry, no truncation)" if it.get("list") else ""
        return f"- {it['col']}: {it['question']}{extra}"
    field_lines = "\n".join(fline(it) for it in fields)
    ...
    # Guided decoding (vLLM): force schema-valid JSON with exactly the expected
    # keys. Makes weak-model output parseable BY CONSTRUCTION — no repair needed.
    # Harmless-to-ignore on backends that don't support it; disable with
    # FCG_GUIDED=0 if your gateway rejects extra_body.
    if GUIDED:
        schema = {
            "type": "object",
            "properties": {it["col"]: {"type": "string"} for it in fields},
            "required": [it["col"] for it in fields],
            "additionalProperties": False,
        }
        payload["guided_json"] = schema
        payload["extra_body"] = {"guided_json": schema}  # some gateways nest it

    headers = {"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"}
    last_err = None
    for attempt in range(1, RETRIES + 1):
        try:
            r = requests.post(url, json=payload, headers=headers, verify=TLS_VERIFY, timeout=HTTP_TIMEOUT)
            ...
            return parse_json_answer(str(content), [it['col'] for it in fields])
...
def save_checkpoint(ck):
    """Thread-safe: unique tmp per writer + atomic rename, guarded by a lock so
    concurrent batches can't race each other (the checkpoint.tmp crash)."""
    CHECKPOINT_PATH.parent.mkdir(parents=True, exist_ok=True)
    tmp = CHECKPOINT_PATH.with_suffix(f".tmp{threading.get_ident()}")
    tmp.write_text(json.dumps(ck), encoding="utf-8")
    tmp.replace(CHECKPOINT_PATH)


def batch_key(country, phase, batch_idx, fields, dossier):
    fh = sha("|".join(it["col"] for it in fields))
    dh = sha(dossier)
    ...
    if todo:
        ...
        with cf.ThreadPoolExecutor(max_workers=CONCURRENCY) as ex:
            ...
            for fut in cf.as_completed(futs):
                bi, b, k = futs[fut]
                ans = fut.result()  # raise -> caught by scan_chunked; chunk skipped
                with CK_LOCK:
                    ck[k] = ans
                    results.update(ans)
                    save_checkpoint(ck)
...
def chunk_text(text: str, size: int, overlap: int):
    """Split text into ~size-char chunks on line boundaries, with overlap so an
    answer straddling a boundary still appears whole in at least one chunk."""
    if len(text) <= size:
        return [text]
    lines = text.split("\n")
...
def merge_answers(dest: dict, new: dict):
    """Join answers found in different chunks. First value wins for identical or
    contained text; genuinely new text is appended with ' | '."""
    for k, v in new.items():
        v = (v or "").strip()
        if not v or v == "NA":
            continue
        cur = (dest.get(k) or "").strip()
        if not cur or cur == "NA":
            dest[k] = v
            continue
        cn, vn = norm_ans(cur), norm_ans(v)
        if vn in cn or cn in vn:
            # keep the longer, more complete rendering
            if len(vn) > len(cn):
                dest[k] = v
            continue
        if len(cur) + len(v) < 4000:      # don't let a field balloon forever
            dest[k] = f"{cur} | {v}"
    return dest
...
def scan_chunked(country, phase, text, fields, ck, label):
    """Run every field batch against EVERY chunk of text, joining what each finds.
    This sees the whole document (no routing narrowness) while keeping each call
    small enough to be fast and not time out."""
    chunks = chunk_text(text, CHUNK_SIZE, CHUNK_OVERLAP)
    log(f"  {country} {phase}: {len(chunks)} chunk(s) x "
        f"{max(1, (len(fields)+BATCH_SIZE-1)//BATCH_SIZE)} batch(es) over {label}")
    merged = {it["col"]: "NA" for it in fields}
    for ci, ch in enumerate(chunks):
        d = (f"### SOURCE: {label} — PART {ci+1} of {len(chunks)} ###\n{ch}")
        try:
            got = run_batches(country, f"{phase}c{ci}", d, fields, ck)
        except BudgetExceeded:
            raise                      # budget must still stop the run
        except Exception as e:
            log(f"    {country} {phase} chunk {ci+1}/{len(chunks)} FAILED "
                f"({str(e)[:120]}) — continuing; its fields stay NA for this chunk")
            continue
        merge_answers(merged, got)
    return merged
...
def process_country(fcg_file, items, codes_map, airports_by_a2,
    ...
    try:
        doc = json.loads(fcg_file.read_text(encoding="utf-8", errors="replace"))
    except Exception as e:
        die(f"Failed to parse FCG file {fcg_file}: {e}")
    ...
    for it in items:
        if it["type"] != "extract":
            ...
            extract_items.append(it)
    n_wf = len(items) - len(extract_items)
    if n_wf:
        log(f"  {country}: {n_wf} non-extract items skipped (no document answer exists)")

    # ---- Phase 1: chunked scan of the WHOLE FCG document (no routing narrowness)
    DOSSIER_DIR.mkdir(parents=True, exist_ok=True)
    full_dossier = cap(fcg_text_for(fcg_file), BUDGET_FCG, "FCG")
    (DOSSIER_DIR / f"{country}_phase1.txt").write_text(full_dossier, encoding="utf-8")
    p1 = scan_chunked(country, "P1", full_dossier, extract_items, ck,
    ...
    # ---- Phase 2: ancillary fill of NA extract fields only (also chunked)
    na_fields = [it for it in extract_items if row[it["col"]] == "NA"]
    if na_fields:
        ...
        if dossier2_body.strip():
            (DOSSIER_DIR / f"{country}_phase2.txt").write_text(dossier2_body, encoding="utf-8")
            p2 = scan_chunked(country, "P2", dossier2_body, na_fields, ck,
                              f"ANCILLARY SOURCES (MEIS + SVC_RMK) — {disp}")
            for it in na_fields:
                v = p2.get(it["col"], "NA")
                if v != "NA":
                    row[it["col"]] = v
            ...
            log(f"  {country}: Phase 2 filled "
                f"{sum(1 for it in na_fields if row[it['col']] != 'NA')}/{len(na_fields)} NA fields")
        else:
            log(f"  {country}: no ancillary data found (ICAOs tried: {len(icaos)})")
    else:
        log(f"  {country}: Phase 1 answered every extract field; skipping Phase 2")

    filled = sum(1 for it in extract_items if row[it["col"]] != "NA")
    log(f"  {country}: FILLED {filled}/{len(extract_items)} extractable fields")
...
def main():
    ...
    ap.add_argument("--verify", action="store_true",
                    help="One cheap health-check call (URL+key+model+TLS), then exit.")
    args = ap.parse_args()
    ...
    if args.verify:
        try:
            r = requests.post(
                ...
                verify=TLS_VERIFY, timeout=60)
            if r.status_code == 200:
                msg = r.json()["choices"][0]["message"].get("content")
                log(f"[verify] OK — model={MODEL} replied: {msg!r}")
                sys.exit(0)
            log(f"[verify] FAILED: HTTP {r.status_code}: {r.text[:200]}")
        except Exception as e:
            log(f"[verify] FAILED: {type(e).__name__}: {e}")
        log("  Check LITELLM_BASE_URL (must end in /v1), LITELLM_API_KEY, "
            "AGENT_MODEL (exact id from /v1/models), and TLS (AGENT_TLS_VERIFY).")
        sys.exit(1)

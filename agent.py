...
TURN_MAX_TOKENS = int(os.environ.get("AGENT_MAX_TOKENS", "16000"))
TOOL_ROUNDS = int(os.environ.get("AGENT_TOOL_ROUNDS", "40"))
...
TOOLS = [
    ...
    {"type": "function", "function": {
        ...
        "description": ("Create or overwrite a file inside the repo with the "
                        "given content. Overwrites keep a .bak backup. For "
                        "LARGE files (>200 lines), write the first section "
                        "here then continue with append_file — a single huge "
    ...
    {"type": "function", "function": {
        "name": "append_file",
        "description": ("Append content to an existing repo file. Use with "
                        "write_file to build large files in sections."),
        "parameters": {"type": "object", "properties": {
            "path": {"type": "string"},
            "content": {"type": "string"}},
            "required": ["path", "content"]}}},
...
def tool_write_file(rel_path: str, content: str) -> str:
    if not rel_path or not rel_path.strip():
        return "REFUSED: empty path. Provide a relative file path."
    target = _confine(rel_path)
    if target is None:
        return "REFUSED: path escapes the repo."
    if target.is_dir():
        return f"REFUSED: {rel_path} is a directory, not a file."
...
def tool_append_file(rel_path: str, content: str) -> str:
    if not rel_path or not rel_path.strip():
    ...
    if target is None:
        return "REFUSED: path escapes the repo."
    if target.is_dir():
        return f"REFUSED: {rel_path} is a directory."
    if target.name in PROTECTED_FILES:
        return "REFUSED: this file is protected."
    if not target.exists():
        return f"REFUSED: {rel_path} does not exist yet — use write_file first."
    with open(target, "a", encoding="utf-8") as f:
        f.write(content)
    return f"Appended {len(content)} chars to {rel_path} " \
           f"(now {target.stat().st_size} bytes)."
...
DISPATCH = {
    ...
    "write_file": lambda a: tool_write_file(a.get("path", ""),
                                            a.get("content", "")),
    "append_file": lambda a: tool_append_file(a.get("path", ""),
                                              a.get("content", "")),
...
def _read_user() -> str:
    """Read a full (possibly multi-line pasted) instruction.

    input() only returns the first line of a paste; the rest stays buffered
    and would leak into later prompts as ghost turns. Drain any immediately
    available lines into this one message.
    """
    import select
    try:
        first = input("you> ")
    ...
    except Exception:  # noqa: BLE001 — stdin quirks: fall back to first line
        pass
    return "\n".join(lines).strip()
...
def main() -> None:
    ...
    while True:
        ...
        # tool loop
        for _ in range(TOOL_ROUNDS):  # hard cap on tool rounds per turn
            try:
                ...
                print(f"\nagent> (API call failed: {e} — your session is "
                      f"intact, try again or rephrase)\n")
                break
            ...
            for tc in msg.tool_calls:
                ...
                if truncated:
                    result = ("TOOL ERROR: your arguments were not valid "
                              "JSON — almost certainly truncated because the "
                              "call was too large. Write the file in SMALLER "
                              "sections: write_file for the first part, then "
                              "append_file for each subsequent part.")
                    print("  [tool] ERROR: truncated arguments")
                else:
                    try:
                        result = DISPATCH.get(
                            name, lambda a: f"Unknown tool {name}")(args)
                    except Exception as e:  # noqa: BLE001 — never kill the session
                        result = (f"TOOL ERROR ({type(e).__name__}): {e}. "
                                  f"Check your arguments and try again.")

...
def build_fuzz_target(
    ...
    cmd = ["g++", "-fsanitize=address,undefined", "-g", "-O1", "-fpermissive",
           "-fopenmp",  # needed if project was built with OpenMP (GraphicsMagick)
           "-include", "string.h",
    ...
    try:
        ...
        if r.returncode != 0:
            # Retry: drop OPTIONAL image libs but keep essential ones
            # (gomp, stdc++, z, m, pthread are never dropped)

...
def find_include_dirs(src_dir: str) -> list[str]:
    ...
    # Add ALL directories containing .h files, AND their parents
    # (so that #include <subdir/file.h> works when file.h is in subdir/)
    for h in src_path.rglob("*.h"):
        d = str(h.parent)
        if any(s in d.lower() for s in skip):
            continue
        dirs.add(d)
        # Add parent so <libxml/xmlstring.h> resolves when the header
        # is at include/libxml/xmlstring.h — the include path needs include/
        parent = h.parent.parent
        if parent and str(parent).startswith(str(src_path)):
            if not any(s in str(parent).lower() for s in skip):
                dirs.add(str(parent))

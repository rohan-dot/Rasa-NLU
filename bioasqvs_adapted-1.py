...
def load_prebuilt_indices() -> None:
    ...
    papers_vs  = FAISS.load_local(
        str(INDEX_DIR), emb, index_name="papers_hnsw",
        allow_dangerous_deserialization=True,
    )
    log.info("  Papers: %d chunks", len(paper_docs))
    ...
    snippets_vs  = FAISS.load_local(
        str(INDEX_DIR), emb, index_name="snippets_hnsw",
        allow_dangerous_deserialization=True,
    )
    log.info("  Snippets: %d chunks", len(snippet_docs))
...
def grade_retrieval(state: dict) -> Literal["generate_draft", "rewrite_question", "decompose_question"]:
    ...
    try:
        ...
        resp = RetrievalGrade(relevance=0.0, coverage=0.0, conflict=False, action="REWRITE")

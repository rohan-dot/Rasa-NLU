...
SECTION_START = r"SECTION I: GENERAL ENTRY REQUIREMENTS"   # slice to Sections I + II only
SECTION_END   = r"SECTION III:"                            # stop at Section III heading (keeps I + II)
...
QUESTIONS = [
    {"id": "diplomatic_lead_time",        "question": "Aircraft clearance lead time(s) and validity, found in the 'LEAD-TIME AND VALIDITY' section (Section II.B). Capture EVERY lead-time value with its category/qualifier (e.g. blanket/annual, DV, HAZMAT, military base, unmanned, other). Match on 'lead time' / 'lead-time' / 'validity' - the words 'diplomatic clearance' may NOT appear nearby"},
    {"id": "hazmat",                      "question": "HAZMAT / dangerous-goods clearance requirements"},
...
def build_prompt(text: str) -> str:
    ...
    return (
        ...
        "PRECEDENCE: when the same item appears in more than one place, use the value from the dedicated\n"
        "AIRCRAFT section (e.g. the aircraft diplomatic-clearance lead-time / validity subsection).\n"
        "Ignore values that appear only in maritime, seaport, or personnel/passport contexts, or in\n"
        "cross-references like 'see Section ...'.\n"
...
def build_verify_prompt(text: str, answers: dict) -> str:
    ...
    return (
        ...
        "- Only use 'NA' when the value is genuinely NOT in the text. If a valid value IS present, keep\n"
        "  or correct it - do NOT NA a field merely because you are unsure.\n"
        "- PRECEDENCE: prefer the value in the dedicated aircraft section over one in a maritime/port,\n"
        "  personnel/passport, or cross-reference context.\n"
        f"{list_rule}\n"
        f"SECTION TEXT:\n{text}\n\n"

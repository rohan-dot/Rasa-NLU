...
def process_country(fcg_file, items, codes_map, airports_by_a2,
    ...
    # ---- Phase 1: routed FCG extraction, grouped by identical focused text
    groups = {}  # text -> [items]
    for it in extract_items:
        txt = route_text(doc, it)
        groups.setdefault(txt, []).append(it)

    fallback_items = groups.pop("", [])
    ...
    p1 = {}
    gi = 0
    for txt, g_items in groups.items():
        gi += 1
        d = (f"### SOURCE: FOREIGN CLEARANCE GUIDE ({disp}) — ROUTED SECTION ###\n"
             + cap(txt, BUDGET_FCG, "FCG"))
        audit.append(f"===== GROUP {gi} (items: {', '.join(i['col'] for i in g_items)}) =====\n{d}")
        p1.update(run_batches(country, "P1", d, g_items, ck))
    if fallback_items:
        d = f"### SOURCE: FOREIGN CLEARANCE GUIDE ({disp}) — FULL DOCUMENT ###\n{full_dossier}"
        audit.append(f"===== FALLBACK (no route hits; items: "
                     f"{', '.join(i['col'] for i in fallback_items)}) =====\n{d}")
        p1.update(run_batches(country, "P1F", d, fallback_items, ck))
    (DOSSIER_DIR / f"{country}_phase1.txt").write_text("\n\n".join(audit), encoding="utf-8")
    log(f"  {country}: P1 routed groups={gi}, fallback items={len(fallback_items)}")
    ...
    # ---- Phase 2: ancillary fill of NA extract fields only

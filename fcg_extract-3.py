...
QUESTIONS = [
    # scope: search ONLY inside subtrees whose key matches one of these
    #        (falls back to whole doc if no scope subtree exists in a file).
    #        Prevents personnelTravel/maritime/groundVehicle text from
    #        contaminating aircraft answers.
    # route: key-name candidates. "name$" = exact match, plain = substring.
    {"id": "diplomatic_lead_time",
     ...
     "scope": ["aircraft", "civilContractAircraft"],
     "route": ["leadTimeValidity", "clearance$"]},

    {"id": "overflight",
     "question": "Overflight clearance requirements: lead time, validity, restrictions, and whether overflight is currently suspended/prohibited",
     "scope": ["aircraft", "civilContractAircraft"],
     "route": ["rteFlgtOprtInfo", "clearance$", "leadTimeValidity"]},

    {"id": "hazmat",
     "question": "HAZMAT / dangerous-goods clearance requirements (may appear as a clearance entry of type HAZMAT)",
     "scope": ["aircraft", "civilContractAircraft"],
     "route": ["clearance$", "leadTimeValidity"]},
    ...
    {"id": "entry_exit_airports",
     ...
     "scope": ["aircraft", "civilContractAircraft", "airport"],
     "route": ["enterDepart", "entryExitReq", "airportDetails"],
     "list": True},

    {"id": "airfield_restrictions",
     "question": "Aircraft entry/exit restrictions at specific airfields, authorized tech stops and alternate airports",
     "scope": ["aircraft", "civilContractAircraft", "airport"],
     "route": ["authTechStopAndAltAirport", "additionalAirportInfo", "airportDetails"]},
    ...
    {"id": "operating_hours",
     "question": "Operating hours, navigation/ops information, and holiday closures",
     "scope": ["aircraft", "civilContractAircraft", "airport"],
     "route": ["navAndOtherOpsInfo", "airportDetails"]},

    {"id": "country_specific",
     "question": "Country-specific forms, APACS instructions, or information required for clearance requests",
     "route": ["apacsClrRequest", "apacsReqInst", "clrReqInstr",
               "permissionRequestInstruction", "apacsReqInstGuide"]},

    {"id": "aircard_cash",
     "question": "Whether an Air Card or any cash payment is required",
     "route": ["generalCountryInfo", "additionalAirportInfo"]},
]
...
def _key_match(key, cand):
    """cand 'name$' -> exact (case-insensitive); plain -> substring."""
    k = key.lower()
...
def find_subtrees(obj, candidates, path="$"):
    ...
    def walk(node, p):
        if isinstance(node, dict):
            for k, v in node.items():
                kp = f"{p}.{k}"
                if any(_key_match(k, c) for c in candidates) and isinstance(v, (dict, list, str)):
                    hits.append((kp, v))
                else:
                    walk(v, kp)
        elif isinstance(node, list):
...
def route_text(doc: dict, q: dict) -> tuple:
    ...
    If q has a "scope", route only within matching top subtrees (e.g. the
    aircraft section), falling back to the whole doc when absent."""
    roots = [(("$", doc))]
    if q.get("scope"):
        scoped = find_subtrees(doc, q["scope"])
        if scoped:
            roots = scoped
    hits = []
    for rp, root in roots:
        for p, sub in find_subtrees(root, q["route"], path=rp):
            hits.append((p, sub))
    if not hits:
        return "", []
    parts, paths = [], []

...
def parse_args():
    ...
    p.add_argument("--diff-file", default=None,
                   help="Path to a .diff/.patch file (AIxCC delta challenges)")
    p.add_argument("--base-commit", default=None,
                   help="Git commit to diff against (bug-injecting commit's parent)")
    return p.parse_args()
...
def main():
    ...
    # Discover every executable harness in the build dir (project-agnostic).
    # An OSS-Fuzz/OSS-CRS build dir holds one or more harness executables
    # (no file extension) alongside data files (.zip/.dict/.options).
    def _discover_harnesses(bdir):
        found = []
        p = Path(bdir)
        if not p.is_dir():
            return found
        data_suffixes = {".zip", ".dict", ".options", ".json", ".txt",
                         ".o", ".a", ".so", ".log"}
        # Known utilities that live in OSS-Fuzz build dirs but aren't harnesses
        not_harness = {"llvm-symbolizer", "sancov", "llvm-cov", "llvm-profdata",
                       "jazzer_driver", "jazzer_agent_deploy.jar"}
        for f in sorted(p.rglob("*")):
            if (f.is_file() and os.access(f, os.X_OK)
                    and f.suffix.lower() not in data_suffixes
                    and f.name.lower() not in not_harness):
                found.append(f)
        return found

    all_harnesses = _discover_harnesses(build_dir)
    binary_exists = False
    found_binary = None

    # If user asked for auto/all, or didn't name a real one, use discovery.
    auto_mode = harness_name.lower() in ("auto", "all", "")

    if not auto_mode:
        # Try to honor the requested name exactly first
        for candidate in [
            Path(build_dir) / harness_name,
            Path(build_dir) / f"{harness_name}_fuzzer",
            Path(build_dir) / f"{harness_name}.bin",
        ]:
            if candidate.is_file() and os.access(candidate, os.X_OK):
                binary_exists, found_binary = True, str(candidate)
                break
        if not binary_exists:
            for g in Path(build_dir).rglob(f"*{harness_name}*"):
                if g.is_file() and os.access(g, os.X_OK) and not g.suffix:
                    binary_exists, found_binary = True, str(g)
                    break

    # Auto mode, or requested name not found → use discovered harnesses
    if not binary_exists and all_harnesses:
        found_binary = str(all_harnesses[0])
        binary_exists = True
        if auto_mode:
            log.info("[binary] Auto-discovered %d harness(es): %s",
                     len(all_harnesses), ", ".join(h.name for h in all_harnesses))
        else:
            log.warning("[binary] '%s' not found; auto-using discovered harness '%s'.",
                        harness_name, all_harnesses[0].name)

    if binary_exists:
        log.info("[binary] Primary harness: %s", found_binary)
        build_dir = str(Path(found_binary).parent)
        harness_name = Path(found_binary).name
        # Any other discovered harnesses become background fuzz targets
        extra_harnesses = [str(h) for h in all_harnesses if str(h) != found_binary]
    else:
        extra_harnesses = []
        log.warning("[binary] No harness binary found in %s. Will auto-generate.",
                    args.build_dir)
        # Diagnostic: show exactly what IS in the build dir so it's obvious
        # whether the dir is empty (build/download failed) or just has
        # differently-named files (wrong --target-harness).
        bp = Path(args.build_dir)
        if not bp.exists():
            log.warning("[binary] DIAGNOSTIC: build dir %s DOES NOT EXIST. "
                        "Did 'build-target' run and download-build-output succeed?",
                        args.build_dir)
        elif not bp.is_dir():
            log.warning("[binary] DIAGNOSTIC: %s exists but is not a directory.",
                        args.build_dir)
        else:
            entries = sorted(bp.iterdir())
            if not entries:
                log.warning("[binary] DIAGNOSTIC: build dir %s is EMPTY. "
                            "The build step produced no output — run 'build-target' "
                            "first and confirm download-build-output populated it.",
                            args.build_dir)
            else:
                log.warning("[binary] DIAGNOSTIC: build dir has %d entries but no "
                            "harness matched '%s'. Contents:", len(entries), args.harness)
                for e in entries[:40]:
                    kind = ("exec" if (e.is_file() and os.access(e, os.X_OK))
                            else "dir" if e.is_dir() else "file")
                    log.warning("[binary]   [%s] %s", kind, e.name)
                log.warning("[binary] → Pass one of the [exec] names above as "
                            "--target-harness, or use 'auto'.")

    # ══════════════════════════════════════════════════════════════
    # PHASE 0: BUILD CALL GRAPH (before harness generation so agent can use it)
    # ══════════════════════════════════════════════════════════════
    ...
    # ── Delta-aware analysis (the key AIxCC insight) ──────────────
    from diff_analysis import analyze_diff, get_diff_context, boost_findings
    diff_result = analyze_diff(args.src_dir, args.diff_file, args.base_commit)
    diff_context = get_diff_context(diff_result)
    if diff_result.has_diff:
        log.info("╔══════════════════════════════════════════════════╗")
        log.info("║  DELTA MODE: focusing on %d changed functions",
                 len(diff_result.changed_functions))
        log.info("╚══════════════════════════════════════════════════╝")

    if not binary_exists and llm.is_available():
        log.info("╔══════════════════════════════════════════════════╗")
        log.info("║  No binary — running agentic harness generator  ║")
        log.info("╚══════════════════════════════════════════════════╝")

        from harness_agent import HarnessAgent
        harness_agent = HarnessAgent(llm, args.src_dir, args.output_dir)
        agent_harnesses = harness_agent.run(call_graph=call_graph, max_harnesses=3)

        if agent_harnesses:
            build_dir = str(Path(agent_harnesses[0]).parent)
            harness_name = Path(agent_harnesses[0]).stem
            log.info("Main harness: %s", agent_harnesses[0])

            for h in agent_harnesses[1:]:
                strategies.generated_harnesses.append(h)
                log.info("Additional harness: %s", h)
        else:
            log.error("Agentic harness generation failed.")
    ...
    # ── Generate LLM seed corpus + dictionary (accelerates fuzzing) ──
    from seed_generator import generate_seeds, generate_dictionary
    seed_target_dir = args.seed_dir or str(Path(args.output_dir) / "seeds")
    Path(seed_target_dir).mkdir(parents=True, exist_ok=True)
    n_seeds = generate_seeds(llm, args.src_dir, seed_target_dir, call_graph)
    fuzz_dict = generate_dictionary(llm, args.src_dir, args.output_dir)
    if n_seeds:
        log.info("[seeds] %d LLM-generated seeds ready.", n_seeds)
    if fuzz_dict:
        log.info("[seeds] Dictionary: %s", fuzz_dict)
        # Pass dictionary to the runner if it supports it
        if hasattr(runner, "extra_args"):
            runner.extra_args = runner.extra_args + [f"-dict={fuzz_dict}"]
        elif hasattr(runner, "dict_path"):
            runner.dict_path = fuzz_dict
    ...
    threads = []
    background_harnesses = list(strategies.get_generated_harnesses())
    # Also fuzz any OTHER harnesses discovered in the build dir (e.g. for
    # libxml2: xml is primary, reader/xpath/html/etc. run in parallel).
    for h in extra_harnesses:
        if h not in background_harnesses:
            background_harnesses.append(h)
            log.info("[fuzz] Background harness: %s", Path(h).name)

    for h in background_harnesses:
        t = threading.Thread(
    ...
    # ── Deduplicate crashes (report unique bugs, not noise) ───────
    from crash_dedup import deduplicate, save_unique_crashes
    all_crash_files = []
    for cdir in (output_dir / "crashes").rglob("crash-*"):
        all_crash_files.append(str(cdir))
    for pov in pov_dir.glob("*"):
        all_crash_files.append(str(pov))

    unique_bugs = []
    if all_crash_files and binary_path:
        log.info("[dedup] Deduplicating %d crash inputs...", len(all_crash_files))
        unique_bugs = deduplicate(all_crash_files, binary_path, args.output_dir)
        if unique_bugs:
            save_unique_crashes(unique_bugs, args.output_dir)
    ...
    log.info("=" * 60)
    log.info("gemma-fuzzer v5.0 — FINAL RESULTS")
    log.info("  Total executions:      %d", runner.state.total_execs)
    log.info("  Raw crashes:           %d", total_crashes)
    log.info("  UNIQUE bugs:           %d", len(unique_bugs))
    ...
    if unique_bugs:
        log.info("")
        log.info("Unique bugs found:")
        for c in unique_bugs:
            log.info("  %s in %s (%d dups) %s",

...
llm_answer = _make_llm(temperature=0.2, max_tokens=2048, disable_tools=False)  # no tool_choice override — Gemma suppresses content with it
...
def _rebuild_faiss_from_raw(faiss_path: str, docs: List[Document]) -> "FAISS":
    ...
    # Use proper FAISS() constructor — __new__ bypasses __init__ and
    # leaves _normalize_L2 and other attributes unset, causing AttributeError.
    vs = FAISS(
        embedding_function = _get_embeddings().embed_query,
        index              = raw_index,
        docstore           = docstore,
...
def generate_draft(state: dict) -> dict:
    ...
    ctx = state.get("context") or ""
    # Bypass LangChain ChatOpenAI entirely — it adds hidden params (tools=[],
    # tool_choice, etc.) that cause Gemma/vLLM to return empty content.
    # Use the raw openai client directly, same as a working direct call.
    from openai import OpenAI as _OpenAI
    _client = _OpenAI(base_url=LLM_BASE_URL, api_key=LLM_API_KEY)
    prompt = (
        "You are a biomedical expert. Answer using ONLY the retrieved snippets below.\n"
        "- Only use information explicitly stated in the snippets.\n"
        "- If the answer is not in the snippets, say \'Not found in the provided evidence.\'\n"
        "- Cite evidence as [1], [2], etc.\n"
        "- For yes/no questions: answer yes/no first, then explain.\n"
        "- For factoid questions: give the specific answer entity first.\n"
        "- For list questions: list all items found, cite each.\n"
        "- For summary questions: synthesize a paragraph from all relevant snippets.\n\n"
        f"Question: {q}\n\n"
    ...
    try:
        ...
        log.info("generate_draft: got %d chars from LLM", len(draft))
    except Exception as e:
        draft = f"LLM error: {type(e).__name__}: {e}"
        log.error("generate_draft LLM call failed: %s", e)
    if not draft:
        draft = "Not found in the provided evidence. (LLM returned empty response)"
    return {"draft_answer": draft, "context": ctx}
...
def reflect(state: dict) -> dict:
    ...
    try:
        ...
        return {"reflect_action": "FINALIZE", "reflect_queries": [],
                ...
                "context": ctx, "draft_answer": draft}
...
def finalize_answer(state: dict, config: Optional[dict] = None) -> dict:
    ...
    # Write memory inline — avoids write_memory_node returning {} which wipes state
    config    = config or {}
    thread_id = (config.get("configurable", {}) or {}).get("thread_id", "default")
    write_longterm_memory(state.get("question") or "", a, thread_id=thread_id)
    # Return full state update so output dict is non-empty
    return {
        "draft_answer"  : a,
...
def build_graph():
    ...
    # write_memory_node removed — memory now written inside finalize_answer
    # to avoid StateGraph(dict) state-wipe bug when node returns {}

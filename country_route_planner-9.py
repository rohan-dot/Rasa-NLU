    ...
    for outer in range(1, 11):
        ...
        if not changed:
            suggestions = suggest_compliant_alternatives(
                ...
                ourairports, nodes, banned_airports, chosen, total_km,
...
def suggest_compliant_alternatives(overridden_stages, stop_countries, pinned,
                                   ...
                                   banned_airports, chosen, total_km,
    ...
    for i, (user_ap, objection) in overridden_stages.items():
        ...
        suggestions.append({
            ...
            "alt_route": alt_chosen, "alt_total_km": alt_total,
            "delta_km": alt_total - total_km,
            "alt_verdict": v})
    return suggestions


def narrate(chosen, total_km, alternates, stop_countries, corridors,
            verdicts, nodes, ourairports, suggestions, travel_date, use_llm):
    def ap_name(ap):
        if ap.startswith("CENTROID:"):
            c = ap.split(":", 1)[1]
    ...
    if suggestions:
        ...
        for s in suggestions:
            ...
            print(f"    revised route: {' -> '.join(s['alt_route'])} "
                  f"({s['alt_total_km']:,.0f} km, "
                  f"{s['delta_km']:+,.0f} km vs your request)")
...
if __name__ == "__main__":
    ...
    chosen_, _, _, _, corridors_, _, nodes_, ourairports_, _ = result
    if args.traffic:

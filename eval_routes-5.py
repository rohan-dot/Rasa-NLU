...
def eval_geometry(pairs, nodes):
    ...
    pd.DataFrame([{"country": c, "missed_count": n} for c, n in
                  conf_fn.most_common()],
                 columns=["country", "missed_count"]).to_csv(
        f"{OUT}/geometry_missed.csv", index=False)
    pd.DataFrame([{"country": c, "overflagged_count": n} for c, n in
                  conf_fp.most_common()],
                 columns=["country", "overflagged_count"]).to_csv(
...
def run_configs(pairs, nodes, graph, blocked, penalty):
    ...
    rows = []
    for i, (a, ca, pa, b, cb, pb) in enumerate(pairs, 1):
        if i % 10 == 0 or i == len(pairs):
            print(f"    [routing] pair {i}/{len(pairs)}")
...
def main():
    ...
    ap.add_argument("--blocks-file", default=None,
                    help="last_run_blocks.json from a planner run: replay the "
                         "routing under the FCG-derived (LLM-judged) blocks")
    ...
    if args.routing:
        ...
        if args.blocks_file:
            fcg_blocks = json.load(open(args.blocks_file))
            extra = set(fcg_blocks.get("blocked_countries", []))
            blocked = blocked | extra
            print(f"[blocks] + {len(extra)} FCG-derived blocks from "
                  f"{args.blocks_file}: {sorted(extra)}")
            summary["fcg_blocks"] = sorted(extra)
